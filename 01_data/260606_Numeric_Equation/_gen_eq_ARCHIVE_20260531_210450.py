...
def gen_cot(d):
    ...
    if jumped:                               # SOLVER halted in §1's concat check -> the query op is fully pinned, answer directly (no §N.3/§N.4, no §2)
        ...
        if n1 == Y:                          # §1 resolved EVERY operator (incl. the query op) -> stop, no §2
            ...
            if res2 is None:                 # §2 illegal -> keep §1
                ...
                L.append(f"Summary: {verdict} Confirm reading order = {conf_reading(mode)}, {dctstr(variants)}")
    ro, rr = mode
    L.append("")
    ...
    def emit_answer(opname, raw, final, ans):    # restate the locked operator as a function def name(a,b)=form, then apply; the leftward digit-reversal is attributed to the READING ORDER, not the operation (kills the concat_rev-vs-reverse-digits conflation)
        ...
        # negative result whose sign is an operator symbol (prefix/suffix): write it in the puzzle's output form.
        # The side is read off the examples; under leftward the digit-reversal makes both placements read back to
        # the value, so we state both candidates and let the example break the tie (rightward is front-only).
        if raw.startswith('-') and fmt in ('prefix', 'suffix'):
        ...
        if rr:                               # leftward reading -> the result is read with its digits reversed (a reading step, SEPARATE from the operation)
            ...
            if raw.startswith('-'):          # leftward leading-minus convention (operator is '-', or no example fixed a side) — state it
                L.append(f"The result is negative; the sign is written as a leading minus, the answer is {ans}.")
    ...
    if seen and variants.get(qop) in ('concatenation', 'reverse concatenation'):
        ...
        if rr:
            L.append(f"{label[qop]}({qta}, {qtb}) = max({qta}, {qtb}) mod min({qta}, {qtb}) = {mx} mod {mn} = {raw}; reading order is {conf_reading(mode)}, so the result {raw} -> {final}; answer = {final}")
        else:
            L.append(f"{label[qop]}({qta}, {qtb}) = max({qta}, {qtb}) mod min({qta}, {qtb}) = {mx} mod {mn} = {raw}; answer = {final}")
