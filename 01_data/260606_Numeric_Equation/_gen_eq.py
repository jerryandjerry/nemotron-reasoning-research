...
"""
import os, re, sys, csv, math, collections, glob
...
CEXPR = {
    ...
    'rsub+1': lambda a, b: f"{b}-{a}+1", 'rsub-1': lambda a, b: f"{b}-{a}-1",
    'rsub+2': lambda a, b: f"{b}-{a}+2", 'rsub-2': lambda a, b: f"{b}-{a}-2",
}
...
CRYPT_NAME = {
    ...
    'rsub+1': 'rsub_signed_plus1', 'rsub-1': 'rsub_signed_minus1', 'rsub+2': 'rsub_signed_plus2', 'rsub-2': 'rsub_signed_minus2',
...
for _n in ('absolute difference', 'negated absolute difference', 'subtraction (a-b)',
           'reverse subtraction (b-a)', 'sub+1', 'sub-1', 'sub+2', 'sub-2',
           'rsub+1', 'rsub-1', 'rsub+2', 'rsub-2'): FAM[_n] = 'sub'
...
EXTRA_OPS = {'add+2': lambda a, b: a + b + 2, 'add-2': lambda a, b: a + b - 2,
             ...
             'rsub+1': lambda a, b: b - a + 1, 'rsub-1': lambda a, b: b - a - 1,
             'rsub+2': lambda a, b: b - a + 2, 'rsub-2': lambda a, b: b - a - 2}
...
CANON_FAM = {'+': 'noisy_add', '-': 'noisy_subtraction', '*': 'noisy_mul'}   # symbol's canonical family — used ONLY for the unseen-operator GUESS (a +/-/* is most likely add/sub/mul); the §n.4 SEARCH order is the symbol-independent GLOBAL_ARITH
GLOBAL_ARITH = ['noisy_add', 'noisy_subtraction', 'noisy_mul']   # the single fixed family traverse order for EVERY symbol; chosen by sweeping all 6 perms for best GT-match (615/732)
...
def mul_parts(a, b):
    """Partial-product pieces of a×b (ints), the proven moe-format split: split the operand with a
    non-zero units digit (prefer the first) into tens+units and distribute. Returns
    (head, mid, tail, product); head/mid/tail are None for a round (units-0) or single-digit factor."""
    p = a * b
    if a >= 10 and a % 10 != 0:
        t, u = (a // 10) * 10, a % 10
        return f"({t}+{u})×{b}", f"{t}×{b} + {u}×{b}", f"{t * b} + {u * b}", p
    if b >= 10 and b % 10 != 0:
        t, u = (b // 10) * 10, b % 10
        return f"{a}×({t}+{u})", f"{a}×{t} + {a}×{u}", f"{a * t} + {a * u}", p
    return None, None, None, p


def decomp_mul(a, b):
    """Partial-product rhs for a×b (ints): the string AFTER 'a×b = ', ending in the product."""
    head, mid, tail, p = mul_parts(a, b)
    return f"{head} = {mid} = {tail} = {p}" if head else f"{p}"


def base_compute(fam, ta, tb):
    """Left-to-right base-op computation line + the base int value, for the ±2-offset families
    (add, mul). `ta, tb` are reading-frame operand strings. The variant is read off the diff
    (target - base) by the caller, so this only ever computes the family BASE (a+b / a×b)."""
    a, b = int(ta), int(tb)
    if fam == 'noisy_add':
        return f"{ta}+{tb} = {a + b}", a + b
    return f"{ta}×{tb} = {decomp_mul(a, b)}", a * b          # noisy_mul


_MUL_SUF = {'multiplication': None, 'multiply+1': ('+', 1), 'multiply-1': ('-', 1),
            'multiply+2': ('+', 2), 'multiply-2': ('-', 2)}


def mul_query(opname, qa, qb):
    """Full decomposed computation for a multiplication-family query op applied to qa,qb (strings):
    'a×b[±k] = (t+u)×b[±k] = t×b + u×b[±k] = .. [±k] = result'. The ±k offset is carried through
    each stage so the final sum is the actual answer."""
    a, b = int(qa), int(qb)
    head, mid, tail, p = mul_parts(a, b)
    suf = _MUL_SUF[opname]
    if suf is None:
        return f"{qa}×{qb} = {head} = {mid} = {tail} = {p}" if head else f"{qa}×{qb} = {p}"
    sgn, k = suf; res = p + k if sgn == '+' else p - k; ks = f" {sgn} {k}"
    if head:
        return f"{qa}×{qb}{sgn}{k} = {head}{ks} = {mid}{ks} = {tail}{ks} = {res}"
    return f"{qa}×{qb}{sgn}{k} = {p}{ks} = {res}"
...
def op_fams_ordered(sym):                                         # symbol-INDEPENDENT now: one global order for all symbols
    return GLOBAL_ARITH + ['concat']                              # -> prior block, candidate state lines, §n.4 traverse, and the unseen guess all use the same order
...
def gen_cot(d):
    ...
    L = []
    # opening
    L.append("Each symbol is a hidden operator. I work out each operator from the example equations, "
             "then apply the QUERY's operator to the QUERY operands and return the answer.")
    L.append("")
    # assign letters (operands are numbers, so only the operator symbols get letters)
    L.append("Denote the operators as unknown functions:")
    ...
    for i, e in enumerate(raw_exs, 1):
        ...
        # mechanical scan-and-replace: every operator symbol -> its letter (incl. one in the rhs). A symbol in
        # the output (e.g. [7) becomes its letter (g7); its NEGATIVE-SIGN meaning is NOT derived yet (prior #4 /
        # sign-check come later), so it is NOT written as -7 here. Forward-only.
        olet = ''.join((f" {label[c]} " if c in label else c) for c in o)   # symbol -> ' letter ' (concat; digits kept as runs)
        olet = ' '.join(olet.split())               # collapse the spaces: a replaced operator-letter is spaced from the digits (f 83, g 7), like the operator between operands (86 g 93)
        L.append(f"  EX{i} {sa}{op}{sb} = {o} becomes {sa} {label[op]} {sb} = {olet}")
    ...
    L.append("1. We only consider four kinds of operations — noisy_addition (~add), noisy_subtraction (~sub), "
             "noisy_multiplication (~mul), and noisy_concatenation (~concat). They are noisy because the result "
             "has a tolerance of ±2 from the exact value and — for subtraction — may also appear as its absolute "
             "value or operand-reversed form. Specifically:")
    L.append("   ~add = [a+b, a+b±1, a+b±2], ~sub = [|a-b|, a-b, -|a-b|, b-a, a-b±1, a-b±2, b-a±1, b-a±2], "
             "~mul = [a×b, a×b±1, a×b±2], ~concat = [ab, ba], all ordered by frequency. "
             "Any other operation is deemed exotic and we will try max(a,b) mod min(a,b).")
    ...
    L.append("6. If the QUERY operator only appears in the QUERY, we cannot solve it from the examples; "
             "we call it a QUERY-only operator. If the QUERY operator appears in the examples but cannot "
             "be solved, it is outside the system, and we call it an exotic operator. In either case "
             "we still resolve it to a concrete answer.")
    # (REMOVED the old "6. For the operators in this puzzle:" block — after the single-global-order change it
    #  printed the identical constant order once per operator, redundant with prior #1 and the §n.1 state line.)
    L.append("")
    ...
    def attempt(m, n):
        ro2, rr2 = m
        if not (ro2 or rr2):                                             # rightward = read left-to-right (tens digit first)
            rdir = "rightward"
        elif fmt == 'suffix':                                           # back-sign rhs -> reverse the WHOLE rhs so the sign flips to the front (legal leftward)
            rdir = "leftward fully, so the negative sign flips to the front"
        elif fmt in ('prefix', 'minus'):                                # front-sign rhs -> reverse the digits only, the sign stays in front
            rdir = "leftward on digit only, so the negative sign stays in the front"
        else:                                                           # no sign in any rhs
            rdir = "leftward on digit only, no negative sign in rhs"
        ...
        ftag = lambda c: PRIOR_TAG.get(c, cexpr(c, 'a', 'b'))          # family -> ~tag; a locked concat variant -> its concat_fwd/rev name
        dc_exotic = set()                                    # ops left with NO family after the digit-count filter + distinctness -> exotic (populated in §n.3)
        def cell(op):                                        # candidate list (ordered, try-order) -> [..]; the unseen QUERY operator stays a bare `unknown`; a digit-count+distinctness-exhausted op shows exotic
            if op in dc_exotic:
                return f"{label[op]} is an exotic operator"
            if not seen and op == qop:
                return f"{label[op]} is a QUERY-only operator"
        ...
        def lock_concat(op):
            ...
            if decider is None:                              # no example has distinct operands -> op is concat, but the direction is undecidable
                ...
                lines.append(f"    {label[op]}: a∥b={fwd}, b∥a={rev}. rhs={tg0}. Both match{more}; no example has distinct operands to fix the direction, so default to a∥b (the more frequent direction).")
            ...
            for (sa, sb, o, i) in deferred:                  # there IS a later decider, so "check the next" is honest
                ...
                lines.append(f"    {label[op]}: EX{i} operands {rev_if(sa, ro2)}, {rev_if(sb, ro2)} are equal, so a∥b = b∥a = {fwd} = output — this example can't decide the direction; check the next.")
            sa0, sb0, o0, i0 = decider
            fwd, rev, tg0 = cf(sa0, sb0, o0)
            head = f"    {label[op]}: a∥b={fwd}, b∥a={rev}. rhs={tg0}. "
            ...
            lines.append(head + f"{'a∥b' if var == 'concatenation' else 'b∥a'} matches -> so lock {label[op]} = {cexpr(var, 'a', 'b')}.")
            ...
            if confirm:
                ...
                if failed:
                    lines.append(f"    {label[op]} also appears in {exl}; check {body} — {cexpr(var, 'a', 'b')} fails.")
                    return None, lines
        ...
        for op in order:
            ...
            if op in confirmed_sub:                  # already ~sub by its output sign -> a signed output can't be concat; skip the check
                L.append(f"    {label[op]}: already ~sub by its output sign, skip the concat check.")
                continue
        ...
        L.append(f"    {state()}")          # (dropped "All N operator(s) remaining are arithmetic" — redundant with the state line, and it over-claimed "arithmetic" for ops that turn out exotic)
        L.append("")
        ...
        # an operator with NO surviving family — the digit count allows only a family distinctness has already
        # taken (e.g. a sign-confirmed ~sub), so no operation remains -> EXOTIC. Pull these out before the
        # assignment loop so §n.4 never indexes an empty candidate list; narrate via the distinct-operator rule.
        for op in sorted(arith, key=lambda o: oidx[o]):
            if not survc[op]:
                dc_exotic.add(op); cand[op] = []
                L.append(f"    {label[op]}: {clause[op]}.")
                L.append(f"      The digit-count filter leaves only ~sub, but by the distinct operator rule "
                         f"{label[op]} cannot be ~sub, so no operation remains and {label[op]} is an exotic operator.")
        remaining = [op for op in arith if op not in dc_exotic]
        while remaining:
            ...
            if sub_ops:                              # sign already pinned it -> HARD
                ...
                L.append(f"    {label[op]}: {clause[op]}.")
                L.append(f"      Surviving candidates: {', '.join(PRIOR_TAG[f] for f in survc[op])}. "
                         f"Try {label[op]} = {PRIOR_TAG[fam]} first ({PRIOR_TAG[fam]} comes first in the operation order).")
        ...
        dead = set(dc_exotic)                        # pre-dead: digit-count+distinctness-exhausted ops (§n.3 -> exotic) + ops that exhaust during the search
        while True:
            asg = []
            for op in order:
                if op in concat_ops:  asg.append(f"{label[op]} = {cexpr(variants[op], 'a', 'b')}")
                elif op in locked:    asg.append(f"{label[op]} = {cexpr(locked[op], 'a', 'b')}")
                elif op in dead:      asg.append(f"{label[op]} is an exotic operator")
                else:                 asg.append(f"{label[op]} = {FAMNAME_TAG[cand[op][idx[op]]]}")
            if not seen: asg.append(f"{label[qop]} is a QUERY-only operator")
            ...
            for op in arith:
                ...
                L.append(f"      We now know all the operands in EX{i0}: {tag}({ta0}, {tb0}) = {tg0}.")
                ...
                if famfit:                           # this family fits -> lock its simplest fitting variant
                    chosen = famfit[0]
                    if fam in ('noisy_add', 'noisy_mul'):
                        # ±2-offset family: compute the BASE op left-to-right, the variant falls out of the
                        # diff (target - base). Forward-only: base is computed, then target-base, then lock.
                        head, base = base_compute(fam, ta0, tb0)
                        d = int(tg0) - base; sd = f"{d:+d}" if d else "0"
                        if d == 0:
                            L.append(f"        {head}; rhs {tg0}; match, so lock {label[op]} = {cexpr(chosen, 'a', 'b')}.")
                        else:
                            L.append(f"        {head}; rhs {tg0}; {tg0}-{base} = {sd}, so lock {label[op]} = {cexpr(chosen, 'a', 'b')}.")
                    else:                            # ~sub: walk the variant list in order, exact-compare each to the rhs;
                        #    the first that matches locks. |a-b|/-|a-b| are exact (no noise); a-b/b-a and their
                        #    ±k offsets are later list entries, each its own exact candidate (no inline tolerance).
                        L.append("        for ~sub, we check these in order [|a-b|, a-b, -|a-b|, b-a], and lock the first match.")
                        for v in fam_variant_order(fam):
                            val = raw_on(sa0, sb0, v, ro2); ev = cexpr(v, ta0, tb0)
                            if v == chosen:
                                L.append(f"        {ev} = {val}; rhs {tg0}; match, so lock {label[op]} = {cexpr(v, 'a', 'b')}.")
                                break
                            if val != tg0:
                                L.append(f"        {ev} = {val}; rhs {tg0} — no.")
                            else:                    # matches the entry but isn't `chosen` -> tentative lock, ruled out by a later example
                                L.append(f"        {ev} = {val}; rhs {tg0}; match, so lock {label[op]} = {cexpr(v, 'a', 'b')}.")
                                for (sa, sb, o, i) in items[op][1:]:
                                    r = raw_on(sa, sb, v, ro2); tg = target_of(o, rr2)
                                    if r != tg:
                                        L.append(f"        {label[op]} also appears in EX{i}; check EX{i}: {cexpr(v, rev_if(sa, ro2), rev_if(sb, ro2))} = {r}; rhs {tg} — {cexpr(v, 'a', 'b')} fails.")
                                        break
                    rest = items[op][1:]                 # confirm — explicit operation form; the mul family shows full partial products (like the lock row and the query answer), every family ends "= result"
                    if rest:
                        exl = ", ".join(f"EX{i}" for (_, _, _, i) in rest)
                        def confirm_expr(sa, sb):
                            ta, tb = rev_if(sa, ro2), rev_if(sb, ro2)
                            return mul_query(chosen, ta, tb) if FAM.get(chosen) == 'mul' else f"{cexpr(chosen, ta, tb)} = {raw_on(sa, sb, chosen, ro2)}"
                        vs = "; ".join(f"EX{i}: {confirm_expr(sa, sb)}" for (sa, sb, o, i) in rest)
                        L.append(f"        {label[op]} also appears in {exl}; check {vs}. Confirmed.")
                    locked[op] = chosen
                elif fam in ('noisy_add', 'noisy_mul'):   # ±2-offset family misses -> diff-form rejection (beyond ±2)
                    head, base = base_compute(fam, ta0, tb0)
                    d0 = int(tg0) - base; sd0 = f"{d0:+d}" if d0 else "0"
                    if abs(d0) > 2:                  # EX1 itself is out of band
                        L.append(f"        {head}; rhs {tg0}; {tg0}-{base} = {sd0}; |{sd0}| > 2, beyond ±2 — no.")
                        L.append(f"        no {tag} variant gives {tg0}, so {label[op]} = {tag} fails.")
                    else:                            # EX1 in band -> lock its variant, then a later example breaks it
                        v0 = first_variant_match(sa0, sb0, o0, fam, ro2, rr2)
                        if d0 == 0:
                            L.append(f"        {head}; rhs {tg0}; match, so lock {label[op]} = {cexpr(v0, 'a', 'b')}.")
                        else:
                            L.append(f"        {head}; rhs {tg0}; {tg0}-{base} = {sd0}, so lock {label[op]} = {cexpr(v0, 'a', 'b')}.")
                        for (sa, sb, o, i) in items[op][1:]:
                            tg2 = target_of(o, rr2)
                            if raw_on(sa, sb, v0, ro2) == tg2: continue
                            h2, b2 = base_compute(fam, rev_if(sa, ro2), rev_if(sb, ro2))
                            d2 = int(tg2) - b2; sd2 = f"{d2:+d}" if d2 else "0"
                            if abs(d2) > 2:
                                L.append(f"        {label[op]} also appears in EX{i}; check EX{i}: {h2}; rhs {tg2}; {tg2}-{b2} = {sd2}; |{sd2}| > 2, beyond ±2 — no.")
                                L.append(f"        no {tag} variant gives {tg2}, so {label[op]} = {tag} fails.")
                            else:                    # EX{i} needs a different offset variant — one operation can't be both
                                w = first_variant_match(sa, sb, o, fam, ro2, rr2)
                                L.append(f"        {label[op]} also appears in EX{i}; check EX{i}: {h2}; rhs {tg2}; {tg2}-{b2} = {sd2} needs {cexpr(w, 'a', 'b')}, not {cexpr(v0, 'a', 'b')}, so {label[op]} = {tag} fails.")
                            break
                    missed.append(op)
                else:                                # ~sub family misses -> walk the list, exact-compare each (no "far from")
                    v0 = first_variant_match(sa0, sb0, o0, fam, ro2, rr2)
                    L.append("        for ~sub, we check these in order [|a-b|, a-b, -|a-b|, b-a], and lock the first match.")
                    if v0 is None:                   # the FIRST example is already out of band for the whole family
                        for v in fam_variant_order(fam)[:4]:   # the 4 structural variants in order: |a-b|, a-b, -|a-b|, b-a (the ±k miss too)
                            L.append(f"        {cexpr(v, ta0, tb0)} = {raw_on(sa0, sb0, v, ro2)}; rhs {tg0} — no.")
                        L.append(f"        no {tag} variant gives {tg0}, so {label[op]} = {tag} fails.")
                    else:                            # EX{i0} forces v0; walk to it (lock), then verification breaks on a later example
                        for v in fam_variant_order(fam):
                            val = raw_on(sa0, sb0, v, ro2); ev = cexpr(v, ta0, tb0)
                            if v == v0:
                                L.append(f"        {ev} = {val}; rhs {tg0}; match, so lock {label[op]} = {cexpr(v, 'a', 'b')}.")
                        ...
                        parts = [f"EX{i}: {e} = {r}" for (i, e, r) in passes] + [f"EX{fi}: {fexpr} = {fr}; rhs {ftg}"]
                        L.append(f"        {label[op]} also appears in {exl}; check {'; '.join(parts)} — no.")
                        if w is None:                # EX{fi} is out of band for the whole family
                            ...
                            L.append(f"        {wexpr} = {ftg} needs {cexpr(w, 'a', 'b')}, not {cexpr(v0, 'a', 'b')}, so {label[op]} = {tag} fails.")
                    missed.append(op)
            if not missed:
                break
            ...
            if gone:
                tail = (" (" + ", ".join(label[op] for op in gone) + (" has" if len(gone) == 1 else " have") +
                        " no candidate family left, so " + ("it is an exotic operator" if len(gone) == 1 else "they are exotic operators") + ")")
            if adv:
                ...
                L.append(f"    No candidate family is left for {', '.join(label[op] for op in gone)}; "
                         f"so {'it is an exotic operator' if len(gone) == 1 else 'they are exotic operators'}.")
                break
        for op in arith:
    ...
    def conf_reading(m):                 # confirm label: a leftward reading carries its mode (digit-only vs fully)
        if not (m[0] or m[1]): return "rightward"
        return "leftward fully" if fmt == 'suffix' else "leftward on digit only"
    ...
    dctstr = lambda vmap: ", ".join((f"{label[op]} = {cexpr(vmap[op], 'a', 'b')}" if vmap.get(op) else (f"{label[op]} is a QUERY-only operator" if (not seen and op == qop) else f"{label[op]} is an exotic operator")) for op in full_order)
    ...
    if res1 is None:                         # §1's equations are malformed -> §1 is illegal; the other reading is the only legal one
        ...
        L.append(f"Summary: §1 is illegal, so §2 is the only legal reading. "
                 f"Confirm reading order = {conf_reading(other)}, {dctstr(variants)}")
    else:
        ...
        if n1 == Y:                          # §1 resolved EVERY operator (incl. the query op) -> stop, no §2
            ...
            L.append("")
            L.append(f"Summary: Confirm reading order = {conf_reading(start)}, {dctstr(variants)}")
            ...
            if res2 is None:                 # §2 illegal -> keep §1
                ...
                L.append(f"Summary: §2 is illegal, so §1 is the only legal reading. "
                         f"Confirm reading order = {conf_reading(start)}, {dctstr(variants)}")
            else:                            # both legal -> reading-decision cascade, built FORWARD with short-circuit:
                #   Gate 1: query operator solved in exactly one reading -> take it (Gate 2/3 never evaluated)
                #   Gate 2: else -> the reading that solved MORE operators wins
                #   Gate 3: else (count tie) -> §1 (the default reading)
                # the verdict string is ACCUMULATED clause-by-clause as each gate is reached, never composed in one shot.
                c2, a2, v2 = res2; n2 = nres(v2)
                L.append(f"  Conclusion of step §2: {{{dctstr(v2)}}}, {n2} of {Y} resolved.")
                L.append("")
                q1 = variants.get(qop) is not None        # query operator solved in §1?
                q2 = v2.get(qop) is not None              # query operator solved in §2?
                ql = label[qop]
                if q1 != q2:                              # Gate 1 DECIDES -> stop; Gate 2/3 are never evaluated
                    w, lo = (2, 1) if q2 else (1, 2)
                    s = f"§{w} solved the QUERY operator {ql}, which §{lo} did not, so §{w} is preferred."
                    if q2: mode = other; concat_ops, arith, variants = c2, a2, v2
                else:                                     # Gate 1 inconclusive -> record its finding, then check Gate 2
                    s = ("Both readings solve" if q1 else "Neither reading solves") + f" the QUERY operator {ql};"
                    if n1 != n2:                          # Gate 2 DECIDES
                        w, lo = (2, 1) if n2 > n1 else (1, 2)
                        s += f" §{w} solved more operators, so §{w} is preferred."
                        if n2 > n1: mode = other; concat_ops, arith, variants = c2, a2, v2
                    else:                                 # Gate 3
                        s += " §1 and §2 solved the same number of operators, so §1 is preferred."
                L.append(f"Summary: {s} Confirm reading order = {conf_reading(mode)}, {dctstr(variants)}")
    ro, rr = mode
    L.append("")
    ...
    def emit_answer(opname, raw, final, ans):    # restate the locked operator as a function def name(a,b)=form, then apply; the leftward digit-reversal is attributed to the READING ORDER, not the operation (kills the concat_rev-vs-reverse-digits conflation)
        L.append(f"Now solve the QUERY {label[qop]}({qta}, {qtb}), applying {label[qop]}(a, b) = {cexpr(opname, 'a', 'b')},")
        # multiplication is computed left-to-right by partial products (fix #1); other ops apply directly
        comp = mul_query(opname, qta, qtb) if FAM.get(opname) == 'mul' else f"{cexpr(opname, qta, qtb)} = {raw}"
        head = f"{label[qop]}({qta}, {qtb}) = {comp}"
        # negative result whose sign IS the operator's symbol — a glyph (prefix/suffix) OR the literal '-' (fmt 'minus').
        # ONE uniform two-step: step 1 reverses to the stand-in (leftward), step 2 maps the minus back to the operator's
        # symbol (identity when that symbol is '-'). The §2 mode already fixed which side the sign sits on.
        if raw.startswith('-') and fmt in ('prefix', 'suffix', 'minus'):   # 'minus' = the operator's symbol literally IS '-' (same situation as a glyph sign; the map is identity)
            if rr:                                           # leftward: (1) reverse per the declared mode, (2) map the minus to the operator symbol (in place — the §2 mode already fixed the side)
                standin = raw[::-1] if fmt == 'suffix' else final        # 'fully' reverses the whole rhs (sign to the back); 'digit only' reverses the digits, leading minus stays in front
                L.append(f"{head}; reading order is {conf_reading(mode)}, so the result {raw} -> {standin}")
                L.append(f"Need to use the operator symbol for the subtraction sign; mapping the subtraction sign {label[qop]} back to {qop}, so {standin} -> {ans}.")
            else:                                            # rightward: no reversal; (1) the result, (2) map the minus to the operator symbol
                L.append(f"{head};")
                L.append(f"Need to use the operator symbol for the subtraction sign; mapping the subtraction sign {label[qop]} back to {qop}, so {raw} -> {ans}.")
            return
        if rr:                               # leftward reading -> the result is read with its digits reversed (a reading step, SEPARATE from the operation)
            if raw.startswith('-'):          # negative, no example fixed a sign symbol (fmt 'none'): step 1 stops at the stand-in, step 2 finalizes (forward-only — no premature answer=)
                L.append(f"{head}; reading order is {conf_reading(mode)}, so the result {raw} -> {final}")
                L.append(f"The result is negative; the sign is written as a leading minus, so the answer is {ans}.")
            else:
                L.append(f"{head}; reading order is {conf_reading(mode)}, so the result {raw} -> {final}; answer = {final}")
        else:                                # rightward: just output the answer
            L.append(f"{head}; answer = {final}")
    ...
    if seen and variants.get(qop) in ('concatenation', 'reverse concatenation'):
        ...
        emit_answer(hk, raw, final, ans)
    elif seen and variants.get(qop):
        chosen = variants[qop]
        raw = raw_on(qsa, qsb, chosen, ro); final = _rev(raw) if rr else raw
        ans = render_answer(final, fmt, qop)
        emit_answer(chosen, raw, final, ans)
    elif seen:                                           # EXOTIC: query op fails the four families -> the only out-of-system op is max-mod-min, VERIFIED against the examples (its fit may revise the reading)
        # verdict + lock + max-mod-min definition are folded into the single "QUERY operator ... is an exotic
        # operator, so lock ... as max(a,b) mod min(a,b), and verify ..." line below (the per-reading state has
        # already declared the op exotic, so the old "not solvable by the four families / Note:" intro is dropped).
        def mmm_rows(m):                                 # per example: (idx, a, b, max, min, value, target, ok) under reading m
            ro2, rr2 = m; out = []
            for (sa, sb, o, i) in items[qop]:
                a, b = int(rev_if(sa, ro2)), int(rev_if(sb, ro2))
                mx2, mn2 = max(a, b), min(a, b)
                v = (mx2 % mn2) if mn2 else mx2
                tg = target_of(o, rr2)
                out.append((i, a, b, mx2, mn2, v, tg, str(v) == str(int(tg))))
            return out
        def head_line(r):                                # full "max(a,b) mod min(a,b) = mx mod mn = v; rhs tg; match|— no."
            i, a, b, mx2, mn2, v, tg, ok = r
            return f"  EX{i}: max({a}, {b}) mod min({a}, {b}) = {mx2} mod {mn2} = {v}; rhs {tg}" + ("; match." if ok else " — no.")
        def confirm_tail(rows):                          # "f also appears in EX..; check EX..: max(a,b) mod min(a,b) = mx mod mn = v; .. . Confirmed."
            rest = rows[1:]
            if not rest: return None
            return (f"  {label[qop]} also appears in {', '.join('EX'+str(r[0]) for r in rest)}; check "
                    + "; ".join(f"EX{r[0]}: max({r[1]}, {r[2]}) mod min({r[1]}, {r[2]}) = {r[3]} mod {r[4]} = {r[5]}" for r in rest) + ". Confirmed.")
        bothlegal = legal_where == "either §1 or §2"
        conf_rows = mmm_rows(mode)
        if all(r[7] for r in conf_rows) or not bothlegal:    # fits the confirmed reading (or it is the only legal one)
            chosen_m = mode
            L.append(f"QUERY operator {label[qop]} is an exotic operator, so lock {label[qop]} as max(a,b) mod min(a,b), and verify it against the examples in the confirmed reading ({rname(mode)}):")
            L.append(head_line(conf_rows[0]))
            t = confirm_tail(conf_rows)
            if t: L.append(t)
        else:                                                # confirmed reading fails -> the exotic fit reveals the actual reading
            alt = (True, True) if mode == (False, False) else (False, False)
            alt_rows = mmm_rows(alt)
            L.append(f"QUERY operator {label[qop]} is an exotic operator, so lock {label[qop]} as max(a,b) mod min(a,b), and verify it against the examples, starting with the confirmed reading ({rname(mode)}):")
            L.append(head_line(next(r for r in conf_rows if not r[7])))
            L.append(f"The confirmed reading does not match, so check the other reading ({rname(alt)}):")
            L.append(head_line(alt_rows[0]))
            t = confirm_tail(alt_rows)
            if t: L.append(t)
            L.append(f"So the actual reading is {rname(alt)}.")
            chosen_m = alt
        ro, rr = chosen_m; mode = chosen_m
        qta2, qtb2 = rev_if(qsa, ro), rev_if(qsb, ro)
        A, B = int(qta2), int(qtb2); mx, mn = max(A, B), min(A, B)
        raw = str(mx % mn) if mn else str(mx); final = _rev(raw) if rr else raw
        ans = render_answer(final, fmt, qop)
        L.append(f"Now solve the QUERY {label[qop]}({qta2}, {qtb2}):")
        if rr:
            L.append(f"{label[qop]}({qta2}, {qtb2}) = max({qta2}, {qtb2}) mod min({qta2}, {qtb2}) = {mx} mod {mn} = {raw}; reading order is {conf_reading(mode)}, so the result {raw} -> {final}; answer = {final}")
        else:
            L.append(f"{label[qop]}({qta2}, {qtb2}) = max({qta2}, {qtb2}) mod min({qta2}, {qtb2}) = {mx} mod {mn} = {raw}; answer = {final}")
    else:                                                # UNSEEN: query op never appears -> prior-based guess (binary rule)
        L.append(f"Query operator {label[qop]} appears only in the QUERY, so no example fixes it; I will assign it by default.")
        ...
        if not used_fams:
            ...
            hk = 'concatenation'
            L.append(f"No operator is solved arithmetically, so {label[qop]} is ~concat. "
                     f"Let me use {label[qop]} = {cexpr(hk, 'a', 'b')}.")
            raw = raw_on(qsa, qsb, hk, ro); final = _rev(raw) if rr else raw
            ans = render_answer(final, fmt, qop)
            emit_answer(hk, raw, final, ans)
            ...
            canon = CANON_FAM.get(qop)                   # symbol's canonical family: + -> ~add, - -> ~sub, * -> ~mul (None for arbitrary symbols)
            ford = []
            for f in ([canon] if canon else []) + GLOBAL_ARITH:    # the symbol's canonical family FIRST, then the global default order
            ...
            ford_str = ", ".join(tg(f) for f in ford)    # the arithmetic families in try-order (canonical symbol first)
            free_str = ", ".join(tg(f) for f in free)    # those still free after the distinct-operator rule
            present_pairs = [f"{label[op]} = {tg(fam_of(variants[op]))}" for op in arith
                             if variants.get(op) and fam_of(variants[op]) in ARITH_FAMS]
            pres = " and ".join(present_pairs)
            sym_note = f" (the symbol '{qop}' usually means {tg(canon)})" if canon else ""
            intro = (f"At least one operator is solved arithmetically, so {label[qop]} is arithmetic too, "
                     f"so {label[qop]} = [{ford_str}]{sym_note}.")
            if free:
                L.append(f"{intro} Since {pres}, the distinct-operator rule leaves {label[qop]} = [{free_str}], "
                         f"so in order I assign {label[qop]} = {tg(chosen_fam)}.")
            else:
                L.append(f"{intro} Since {pres}, every arithmetic operation is already used, "
                         f"so I take the first, {label[qop]} = {tg(chosen_fam)}.")
            # rule 3: a leading-0 number is invalid for an arithmetic operator. We just guessed the query op is
            # arithmetic, so concat is off the table -> a leading-0 query operand can't be explained, it means the
            # CONFIRMED reading is wrong for this query. Flip to the other reading (when legal and it removes the 0).
            qa, qb = rev_if(qsa, ro), rev_if(qsb, ro)
            bad = next((n for n in (qa, qb) if len(n) > 1 and n[0] == '0'), None)
            alt = (True, True) if mode == (False, False) else (False, False)
            qa2, qb2 = rev_if(qsa, alt[0]), rev_if(qsb, alt[0])
            altok = not ((len(qa2) > 1 and qa2[0] == '0') or (len(qb2) > 1 and qb2[0] == '0'))
            if bad is not None and legal_where == "either §1 or §2" and altok:
                L.append(f"Note: QUERY contains {bad}, a number with a leading 0, which is invalid for an arithmetic operator, so the {rname(mode)} reading is illegal here; read it {rname(alt)}, so QUERY is {label[qop]}({qa2}, {qb2}).")
                ro, rr = alt; mode = alt; qta, qtb = qa2, qb2
            raw = raw_on(qsa, qsb, base, ro); final = _rev(raw) if rr else raw
            ans = render_answer(final, fmt, qop)
            emit_answer(base, raw, final, ans)
...
def load_folder(folder):                                 # build the puzzle record from its question.txt + answer.txt (NO csv)
    prompt = open(os.path.join(folder, 'question.txt')).read()
    answer = open(os.path.join(folder, 'answer.txt')).read()
    ex, qn = parse_prompt(prompt)
    return {'prompt': prompt, 'answer': answer, 'examples': ex, 'question': qn}


def main():
    only = sys.argv[1] if len(sys.argv) > 1 else None    # regenerate a single puzzle id
    folders = sorted(glob.glob(os.path.join(OUT_DIR, 'equation_numeric_*')))
    n = match = 0; by_cat = collections.Counter(); by_cat_ok = collections.Counter()
    for folder in folders:
        if not os.path.isdir(folder): continue
        pid = os.path.basename(folder).replace('equation_numeric_', '')
        if only and pid != only: continue
        d = load_folder(folder)
        lp = os.path.join(folder, 'label.txt')                                # label.txt is a FIXED artifact (set once; may carry a manual DONT_USE). The generator NEVER rewrites it.
        cat = open(lp).read().splitlines()[0].strip() if os.path.exists(lp) else classify(d)   # read the fixed label for the run summary only; classify() is just the fallback for a label-less folder
        cot, ans = gen_cot(d)
        ok = verify(d['answer'], extract_final_answer(cot))
        n += 1; match += ok; by_cat[cat] += 1; by_cat_ok[cat] += ok
        os.makedirs(os.path.join(folder, 'track'), exist_ok=True)
        open(os.path.join(folder, 'track', 'tree_cot.txt'), 'w').write(cot)   # the ONLY file the generator writes: question.txt -> tree_cot.txt
        if only:
            print(f"regenerated equation_numeric_{pid}  (label={cat})")
            print(f"  our answer = {extract_final_answer(cot)} | gold = {d['answer'].strip()} | {'OK' if ok else 'MISS'}")
    if only:
        return
    print(f"updated {n} puzzle CoTs under {OUT_DIR}  (global order = {[FAMNAME_TAG[f] for f in GLOBAL_ARITH]})")
