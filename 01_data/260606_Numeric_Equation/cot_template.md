...
- **Operators we search:** ~add = [a+b, a+b±1, a+b±2], ~mul = [a×b, a×b±1, a×b±2], ~sub = [|a-b|, a-b, -|a-b|, b-a, a-b±1, a-b±2, b-a±1, b-a±2], and ~concat [ab, ba], all ordered by frequency. Any other operation is deemed **exotic**, and we then try `max(a,b) mod min(a,b)` — never fabricated.
...
- **Exotic vs QUERY-only are two DISTINCT verdicts.** An operator that EXHAUSTS all four families (tried every family, none fit) is rendered `[LABEL] is an exotic operator` — it is NOT counted in "X of Y resolved", and a query op then locks `max(a,b) mod min(a,b)`. An operator that NEVER appears in the examples (only in the QUERY, so there is nothing to try) is `[LABEL] is a QUERY-only operator` and is assigned by default. Exhausted ⇒ exotic; never-tried ⇒ QUERY-only operator.
...
```
Each symbol is a hidden operator. I work out each operator from the example equations, then apply the QUERY's operator to the QUERY operands and return the answer.
...
1. We only consider four kinds of operations — noisy_addition (~add), noisy_subtraction (~sub), noisy_multiplication (~mul), and noisy_concatenation (~concat). They are noisy because the result has a tolerance of ±2 from the exact value and — for subtraction — may also appear as its absolute value or operand-reversed form. Specifically:
   ~add = [a+b, a+b±1, a+b±2], ~sub = [|a-b|, a-b, -|a-b|, b-a, a-b±1, a-b±2, b-a±1, b-a±2], ~mul = [a×b, a×b±1, a×b±2], ~concat = [ab, ba], all ordered by frequency. Any other operation is deemed exotic and we will try max(a,b) mod min(a,b).
...
### (No per-symbol operator-prior block — the numbered list ends at item 6)

There is **no "For the operators in this puzzle:" per-symbol block** (item 6 defines the QUERY-only and exotic verdicts, not a per-symbol prior). The family try-order is a single **fixed global order `[~add, ~sub, ~mul, ~concat]`**, used for every operator in the §N.1 state line and the §N.4 search — NOT a per-symbol frequency prior. A symbol's identity matters only for the **unseen-operator by-default assignment** (see Query answer): a `+`/`-`/`*` query operator is assigned `~add`/`~sub`/`~mul` by default; arbitrary symbols fall back to the global order.
...
```
*The second line is emitted ONCE after the sign-confirmed line(s): it lists the EXAMPLE operators that are NOT sign-confirmed — by distinctness (prior #2) they cannot also be ~sub. Omitted when there are no other example operators.*
...
```
*The last line logs the **solution state**: each EXAMPLE operator's candidate-family set, in the fixed global order `[~add, ~sub, ~mul, ~concat]`. The **unseen QUERY operator** (one that appears only in the QUERY) is shown as `is a QUERY-only operator` — it has no examples, so nothing narrows it. A **sign-confirmed** operator collapses to `[~sub]`. (This replaces the old `Unknown:/We know` prose.)*
...
```
  ...
  **Unseen QUERY operator:** a leading 0 on a QUERY operand whose operator appears ONLY in the QUERY has no example to fix the concat direction (fwd/rev undecidable), so it is NOT scanned in §N.1. It is handled later at the by-default step (Query answer (c1)): if the query op is assigned ARITHMETIC by default, a leading-0 operand is invalid for arithmetic (rule 3) → the confirmed reading is illegal → flip to the other reading; if assigned concat, a leading 0 is fine, so it is left as-is.
...
**A concat query operator is locked normally — there is NO early halt.** When the operator that locks concat is the QUERY operator (in §N.2 or the §N.1 leading-0 gut-check), the solver does NOT short-circuit: it still runs §N.3/§N.4 and, if an operator is left unresolved, the other reading, exactly like any other puzzle. (Earlier versions early-exited here on the grounds that concat is reading-invariant; that short-circuit was REMOVED so every CoT runs the full resolution. Boxed answers are unchanged — concat being reading-invariant means the full run reaches the same answer.)
...
- **no family survives (→ EXOTIC):** the digit count allows only a family that distinctness has already taken, so none remains — declared exotic right here and excluded from the §N.4 search (its query op then resolves via max-mod-min): `The digit-count filter leaves only ~sub, but by the distinct operator rule [LABEL] cannot be ~sub, so no operation remains and [LABEL] is an exotic operator.`
...
*(The unseen QUERY operator is NOT pruned or narrated here — it has no examples, so it shows `is a QUERY-only operator` in the state line.)*

```
    No further pruning found. We have f = [~X, ~Y], g = [a∥b] to traverse in order.   // running solution state for EVERY operator: arith ops show their surviving families (tried in this order), a locked concat op shows its variant, the unseen QUERY operator shows `is a QUERY-only operator`
```
...
```
  §[N].4 enter the tree and search for an operator assignment:
    Try operators f = [~X], g = [~Y][, h = [~Z]][, [QLABEL] is a QUERY-only operator]:   // current combo: each unsolved op's first candidate (a concat op shows its locked variant; an op that has EXHAUSTED all families shows `is an exotic operator`; the unseen QUERY operator trails as a QUERY-only operator). Two ops MAY show the same family here (e.g. both ~add) — that's fine, distinctness resolves at lock: one fails and falls through, or one locks and the other drops it.
...
- **family fits** → lock the variant, computed **left-to-right** (the log is a forward computation, not a result-first assertion):
  ...
  - **~sub (structural variants):** first print the order line `for ~sub, we check these in order [|a-b|, a-b, -|a-b|, b-a], and lock the first match.`, then a **left-to-right walk** over the variant order `|a-b| → a-b → -|a-b| → b-a → a-b±1 → a-b±2 → b-a±1 → b-a±2`. The absolute forms `|a-b|`, `-|a-b|` carry no noise; only the signed forms `a-b`, `b-a` get the ±k offsets. Each variant is computed and compared to the rhs **exactly** (no diff/tolerance line — the ±k are their own candidates): `[EXPR] = [VAL]; rhs [TG] — no.` for a mismatch, and the first that equals the rhs locks with **`match`**: `[EXPR] = [VAL]; rhs [TG]; match, so lock [LABEL] = [FORMULA].` (e.g. `|86-93| = 7; rhs -7 — no.` then `86-93 = -7; rhs -7; match, so lock g = a-b.`). No "far from" — the value and the rhs are both shown, so the mismatch is explicit. **Tentative-lock-then-break (within a FITTING ~sub family):** if an EARLIER variant than the chosen one matches the FIRST example, it tentatively locks (`match, so lock …`) and is then broken by a later example with the compact form `[LABEL] also appears in EX[i]; check EX[i]: [EXPR] = [VAL]; rhs [TG] — [FORMULA] fails.` (e.g. `— a-b fails.`, `— |a-b| fails.`), naming the abandoned variant; the walk then continues to the chosen variant. (This is distinct from a plain confirm mismatch, which ends `— no.`)
  - **confirm (all families) — show the operation EXPLICITLY, the SAME form as the lock row:** if the op has other examples, append `[LABEL] also appears in EX[i], EX[j]; check EX[i]: [EXPR] = [VAL]; …. Confirmed.` — +/−/∥ directly (`82+86 = 168`), and **multiplication by FULL partial products** (`57×16-1 = (50+7)×16 - 1 = 50×16 + 7×16 - 1 = 800 + 112 - 1 = 911`), matching the lock row and the query answer (never a collapsed `57×16-1 = 911`).
...
```
    Advance the unsolved operator(s) to the next candidate: g -> [~Y2], h -> [~Z2][ (LABEL has no candidate family left, so it is an exotic operator)].
```
*Repeat until every op locks or exhausts its candidates. An op that exhausts ALL four families is an **exotic operator** (it tried every family and none fit) — named in the `Advance …` tail, or (if no op can advance at all) `No candidate family is left for [LABEL]; so it is an exotic operator.` An exotic operator is NOT counted in the resolved tally. When several operators exhaust in the SAME step, the verdict is batched and plural: `No candidate family is left for f, g; so they are exotic operators.`*

**Conclusion of step §N** *(lists every operator — the example operators AND, when the query operator is unseen, the query operator too — each shown as its resolved variant, `is an exotic operator` (exhausted all four families), or `is a QUERY-only operator` (unseen, only in QUERY). The count `X of Y`: Y counts EVERY listed operator; X counts only the RESOLVED ones — an exotic operator and a QUERY-only operator are both NOT counted as resolved. A step concludes ONLY itself — no cross-reading comparison here (that is the Summary's job). The one exception is §1, which must decide whether to stop or open §2.):*

```
  Conclusion of step §[N]: {f = [VARIANT | is an exotic operator | is a QUERY-only operator], g = [VARIANT | is an exotic operator | is a QUERY-only operator][, h = ...]}, [X] of [Y] resolved.[ This is the answer. | Need to try reading [OTHER DIR].]
```

- **§1 resolved EVERY operator (X == Y, the query op included):** append `This is the answer.` — §2 is not opened.
- **§1 left ANYTHING unresolved (X < Y — a stuck example op, OR the unseen query operator, which is always still a QUERY-only operator):** append `Need to try reading [OTHER DIR].`, then open §2. The rule is UNIFORM: `X of Y` with `X < Y` always triggers the other reading. An unseen query op is counted in Y but is never resolved by the search (it has no examples), so §1 always lands here and §2 is always opened. §2 can't pin the query op either → the readings tie on it → the Summary picks the reading by EXAMPLE-operator count (§1 on a tie), and the query op is assigned by default after the Summary.
...
```
Summary: Confirm reading order = [DIR], f = [VARIANT | is a QUERY-only operator], g = [VARIANT | is a QUERY-only operator][, h = ...]
```
...
```
Summary: [VERDICT] Confirm reading order = [DIR], f = [VARIANT | is a QUERY-only operator], g = [VARIANT | is a QUERY-only operator][, h = ...]
```
...
### Query answer (apply the resolved/assigned operator to the QUERY, in the confirmed reading frame)

*`[QA], [QB]` are the reading-frame query operands (reversed iff leftward — the same ones written in §N.1). The operator's meaning is already stated (in the Summary for a determined op, or in the by-default reasoning below), so this step is just the computation.*
...
```
Query operator g appears only in the QUERY, so no example fixes it; I will assign it by default.
```

- **at least one operator was solved arithmetically** → arithmetic by default. Intro `At least one operator is solved arithmetically, so g is arithmetic too, so g = [~add, ~sub, ~mul]` (arithmetic families in try-order, canonical symbol first), then the distinct-operator narrowing:
  - **distinct-operator narrowing:** `[INTRO] Since [LOCKED, e.g. f = ~add and h = ~mul], the distinct-operator rule leaves g = [~Y, ~Z], so in order I assign g = [~Y].` A canonical symbol (+ → ~add, - → ~sub, * → ~mul) leads the order, noted inline as `(the symbol '[OP]' usually means [~Z])`.
  - **every arithmetic family already used:** `[INTRO] Since [LOCKED], every arithmetic operation is already used, so I take the first, g = [~Z].`
- **no operator was solved arithmetically** → **concatenation** by default, then commit the forward order: `No operator is solved arithmetically, so g is ~concat. Let me use g = a∥b.` The `Now solve the QUERY` line then uses `a∥b`.
  - **leading-0 query operand after an ARITHMETIC by-default assignment:** once the query op is assigned arithmetic, concat is ruled out, so a leading-0 operand in the confirmed reading can't be explained — it is invalid for an arithmetic operator (rule 3), so the confirmed reading is illegal for the query. Flip to the other reading (only when it is legal AND removes the leading 0): `Note: QUERY contains [NUM], a number with a leading 0, which is invalid for an arithmetic operator, so the [DIR] reading is illegal here; read it [OTHER], where the operands are [QA], [QB].` then solve there. (A *concat* assignment does NOT flip — a leading 0 is valid for concatenation.)

For (c1), the assigned operator's value is what `[EXPR]` uses in the `Now solve the QUERY` line (arithmetic base op `a+b` / `|a-b|` / `a×b`, or `a∥b` / `b∥a` for concat).

**(c2) EXOTIC query** (the operator appears in the examples, but every modeled family was disproven for it — it EXHAUSTED all four families in the legal reading(s)): the operator is exotic, and by prior knowledge #1 we try `max(a,b) mod min(a,b)`. It is **locked and VERIFIED against the examples**, starting with the confirmed reading: if it reproduces every example there, that reading stands; if it does NOT *and both readings are legal*, the other reading is checked, and if max-mod-min fits there the **actual reading is revised to it** — an exotic operator can overturn the four-family reading. (`max(a,b) mod min(a,b)` is OBSERVED — across the exotic puzzles the out-of-system operator is almost always max mod min — and is established up front in prior #1, so the verdict line locks it DIRECTLY; the old separate `The only operation that exists outside the system …` introduction and the `Note: an exotic operator may change all the rule …` line are GONE.) If max-mod-min fits NEITHER legal reading the puzzle is genuinely unsolvable (none in the current corpus).

```
QUERY operator g is an exotic operator, so lock g as max(a,b) mod min(a,b), and verify it against the examples in the confirmed reading ([DIR]):
  EX[i]: max([A], [B]) mod min([A], [B]) = [MX] mod [MN] = [V]; rhs [TG]; match.        // "— no." if the confirmed reading fails
  [g also appears in EX..; check EX..: max([A], [B]) mod min([A], [B]) = [MX] mod [MN] = [V]; .. . Confirmed.]   // confirm tail when >1 example — operation shown EXPLICITLY (not bare "[MX] mod [MN]")
[The confirmed reading does not match, so check the other reading ([OTHER]):           // ONLY when confirmed fails AND both readings legal; the lead line then ends "..., and verify it against the examples, starting with the confirmed reading ([DIR]):"
