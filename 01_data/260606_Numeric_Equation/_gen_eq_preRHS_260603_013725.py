...
def gen_cot(d):
    ...
    L.append("   ~add = [a+b, a+b±1, a+b±2], ~sub = [|a-b|, a-b, -|a-b|, b-a, a-b±1, a-b±2, b-a±1, b-a±2], "
             ...
             "The absolute forms |a-b|, -|a-b| carry no noise — they are themselves the unsigned reading of the "
             "signed difference; the ±2 tolerance applies to the signed forms a-b, b-a. "
    ...
    def attempt(m, n):
        ...
        for tag, op, nums in scan:                        # any leading-0 number -> the operator must be concat (prior #3)
            for num in nums:
                if len(num) > 1 and num[0] == '0':
                    ...
                    if seen and op == qop:   # SOLVER HALT: the query op was just forced concat (and confirmed) -> answer directly, skip §N.2/§N.3/§N.4
                        L.append(f"    Note: {label[qop]} is the QUERY operator, and it has been locked as {cexpr(variant, 'a', 'b')}. At this point other unknowns are irrelevant, because the answer is simply the concatenation of the query operands. Proceed to answer directly.")
                        jumped.append(True)
                        return dict(concat_locked), [o for o in order if o not in concat_locked], {o: concat_locked[o] for o in concat_locked}
                    break
        if not seen:
        ...
        for op in order:
            ...
            if variant is not None:
                ...
                if seen and op == qop:   # SOLVER HALT: the query op is all the query needs, and it just locked (and confirmed) concat -> stop searching (skip §N.3/§N.4 and the other reading)
                    L.append(f"    Note: {label[qop]} is the QUERY operator, and it has been locked as {cexpr(variant, 'a', 'b')}. At this point other unknowns are irrelevant, because the answer is simply the concatenation of the query operands. Proceed to answer directly.")
                    jumped.append(True)
                    arith = [o for o in order if o not in concat_ops]
                    return concat_ops, arith, {o: concat_ops[o] for o in concat_ops}
            else:
                            ...
                            if val != tg0:
                                ...
                                for (sa, sb, o, i) in items[op][1:]:
                                    ...
                                    if r != tg:
                                        ...
                                        break
                    rest = items[op][1:]                 # confirm — untouched (variant-expr form), same for every family
                    if rest:
    ...
    dctstr = lambda vmap: ", ".join(f"{label[op]} = {cexpr(vmap[op], 'a', 'b') if vmap.get(op) else 'unknown'}" for op in full_order)
    ...
    jumped = []                              # set by attempt() when its §N.2 halts on the query operator (concat, single example)
    res1 = attempt(start, 1)
    L.append("")
    if jumped:                               # SOLVER halted in §1's concat check -> the query op is fully pinned, answer directly (no §N.3/§N.4, no §2)
        concat_ops, arith, variants = res1; mode = start; nj = nres(variants)
        L.append(f"  Conclusion of step §1: {{{dctstr(variants)}}}, {nj} of {Y} resolved.")
        L.append("")
        L.append(f"Summary: Query operator {label[qop]} = {cexpr(variants[qop], 'a', 'b')}, no need to solve the other unknowns; Confirm reading order = {conf_reading(start)}, {dctstr(variants)}")
    elif res1 is None:                       # §1's equations are malformed -> §1 is illegal; the other reading is the only legal one
        legal_where = "§2"
        res2 = attempt(other, 2)
        L.append("")
        if jumped:                           # SOLVER halted in §2's concat check (§1 was illegal)
            concat_ops, arith, variants = res2; mode = other; nj = nres(variants)
            L.append(f"  Conclusion of step §2: {{{dctstr(variants)}}}, {nj} of {Y} resolved.")
            L.append("")
            L.append(f"Summary: Query operator {label[qop]} = {cexpr(variants[qop], 'a', 'b')}, no need to solve the other unknowns; Confirm reading order = {conf_reading(other)}, {dctstr(variants)}")
        else:
    ...
    if seen and variants.get(qop) in ('concatenation', 'reverse concatenation'):
        ...
        L.append(f"QUERY operator {label[qop]} is not solvable by the four families in {legal_where}, so {label[qop]} is an exotic operator. Note: an exotic operator may change all the rule we deduce above.")
        L.append("The only operation that exists outside the system is max-mod-min(a,b) = max(a, b) mod min(a, b)")
        ...
        def confirm_tail(rows):                          # "f also appears in EX..; check EX..: mx mod mn = v; .. . Confirmed."
            rest = rows[1:]
            if not rest: return None
            return (f"  {label[qop]} also appears in {', '.join('EX'+str(r[0]) for r in rest)}; check "
                    + "; ".join(f"EX{r[0]}: {r[3]} mod {r[4]} = {r[5]}" for r in rest) + ". Confirmed.")
        ...
        if all(r[7] for r in conf_rows) or not bothlegal:    # fits the confirmed reading (or it is the only legal one)
            chosen_m = mode
            L.append(f"Lock {label[qop]} = max-mod-min, and verify it against the examples in the confirmed reading ({rname(mode)}):")
            ...
            L.append(f"Lock {label[qop]} = max-mod-min, and verify it against the examples, starting with the confirmed reading ({rname(mode)}):")
