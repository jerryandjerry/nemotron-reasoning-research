...
> **⏩ 2026-06-07 lesson — "exotic" and "unknown" are NOT the same and must read differently.** Exhausting
> all four families (the search ran and found nothing) is **exotic** → `is an exotic operator`, and the query
> op then locks `max(a,b) mod min(a,b)` (established in prior #1). An operator that never appears in the
> examples (nothing to search) is **unknown** → guessed. Both are excluded from the "X of Y resolved" count.
> A second lesson: **always show the operation explicitly** in confirm rows (mul by partial products, mod by
> max()/min()), the same form as the lock row — never a collapsed `= result`. Full changelog:
> [`CHANGES_260607.md`](CHANGES_260607.md). All 2026-06-07 changes were narration-only (0 box diffs, GT 621).
