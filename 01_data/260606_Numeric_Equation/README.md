...
> **⏩ 2026-06-07 — this is the ACTIVE folder (`260606_Numeric_Equation/`).** `260526_Numeric_Equation/` is
> the FROZEN reference used only for box-diffs. Generator `_gen_eq.py`, spec `cot_template.md` (both synced).
> GT 621/732 (84.8%); every change below is narration-only (0 boxed-answer diffs vs frozen 260526). This
> session's CoT changes are summarized in [`CHANGES_260607.md`](CHANGES_260607.md) (also the hand-off for the
> cryptarithm agent): exotic≠unknown (exhausted families → "is an exotic operator", not counted as resolved;
> unseen-only-in-QUERY → "unknown" → guess), verdict line folded to `… is an exotic operator, so lock … as
> max(a,b) mod min(a,b) …`, max-mod-min established in prior #1, every operation computed EXPLICITLY (mul by
> partial products) in confirm rows too, prior point 6 (never box "unknown"), and the concat-halt
> short-circuit removed (Fix 1 — full resolution always).
