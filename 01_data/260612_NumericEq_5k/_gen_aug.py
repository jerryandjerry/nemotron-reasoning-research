#!/usr/bin/env python3
"""Augmentation generator v3 (260612 5k round) — adds the 5th, never-before-generated type:
ambiguous_arithmetic (see build_ambiguous). Otherwise identical to the hardened 260607 v2.
Folders NE5k_NNNNNN in THIS directory; the build pulls from the 260607 pool AND this one.

v2 design — COMPLEMENT the original 732 (fill the thin tails / OOD), not mirror it.
...
from _gen_eq import parse_prompt, classify, consistent_ops     # gen-time gates for the ambiguous type
...
# ── ambiguous_arithmetic (the 5th type — NEW in the 260612 round) ────────────────────────────────
# The QUERY operator is a sub-family op whose examples sit in ONE sign regime, so >=2 sub variants fit
# them all; the query operands flip the regime, so the fitting variants DISAGREE. Gold = the solver's
# ordered ~sub tie-break pick (|a-b| -> a-b -> -|a-b| -> b-a, lock the first match): for ambiguous the
# gold is underdetermined by the examples — exactly the property of the 36 orig GT-True ambiguous.
# Fillers exclude sub (a second sub-family op would trigger the solver's distinct-operator family
# elimination and kill the ambiguity); the puzzle-level signed-conversion is OFF (sign-ness comes from
# the regime itself). The 4 pair-types and their measured share of the 65 originals:
...
#                                      (gate-safe: the examples already demonstrate the glyph)
AMB_CASES = {
...
AMB_WEIGHTS = [46, 32, 11, 9]

def in_regime(P, Q, regime): return (P > Q) if regime == 'P>Q' else (P < Q)

def build_ambiguous(rng, n_fill=None):
    C = AMB_CASES[rng.choices([1, 2, 3, 4], weights=AMB_WEIGHTS)[0]]
    nf = n_fill if n_fill is not None else rng.choice([1, 2])   # n_fill=0 -> single-operator ambiguous (the sub op only)
    fams = ['sub'] + rng.sample(['add', 'mul', 'concat'], nf); qi = 0
    ...
    for k, (fam, V, sym, c) in enumerate(zip(fams, variants, syms, counts)):
        ...
        for j in range(c):
            for _t in range(400):
                ...
                P = operand(ro, rng, want_leadzero=wz); Q = operand(ro, rng)
                if rng.random() < 0.5 and wz: P, Q = Q, P
                if k == qi:                                      # the ambiguous op: confine to its regime
    ...
    for fam, V, sym in zip(fams, variants, syms):                # every intended variant must reproduce its examples
        if not all(fits(sa, sb, V, ro, rr, o) for (sa, sb, o) in trip[sym]):
            raise Reject()
    fitq = consistent_ops(trip[qsym], ro, rr)                    # gen-time: the solver's FIRST ordered fit must be our pick
    if not fitq or fitq[0] != C['pick']:
        raise Reject()

    for _t in range(400):                                        # query: OPPOSITE regime, pick & dissenter disagree
        Pq = operand(ro, rng); Qq = operand(ro, rng)
    ...
    text = INTRO + "\n" + "\n".join(lines) + f"\nNow, determine the result for: {qsa}{qsym}{qsb}\n"
    ex, qn = parse_prompt(text)                                  # final gate: the solver's own classifier agrees
    if classify({'examples': ex, 'question': qn}) != 'ambiguous_arithmetic':
        raise Reject()
    return text, gold, 'NE_aug_ambiguous_arithmetic', reading, 'ambiguous', signed, fmt, leadzero
...
def main():
    ...
    # 5k round: ABSOLUTE targets (yield-margined: deducible/exotic 99%, unseen 82%, concat 100%,
    # ambiguous ~95% expected). Secondary features still injected at the SAME rate in every category.
    MIX = [('deducible', 1580), ('exotic', 220), ('unseen', 240), ('ambiguous', 560), ('concat', 360)]
    targets = dict(MIX)
    if a.test:
        for p, _ in MIX:
            print(f"\n===== {p} =====")
            for _ in range(a.test):
                r = None
                while r is None:
                    try: r = build_ambiguous(rng) if p == 'ambiguous' else build_puzzle(p, rng)
    ...
    for ptype, _ in MIX:
        ...
        while made < targets[ptype]:
            ...
            try: text, gold, label, reading, pt, signed, fmt, lz = (build_ambiguous(rng) if ptype == 'ambiguous' else build_puzzle(ptype, rng))
            except Reject: continue
            idx += 1; pid = f"NE5k_{idx:06d}"
