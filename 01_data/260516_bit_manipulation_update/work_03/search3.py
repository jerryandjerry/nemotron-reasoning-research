...
def search3(examples):
    ...
    for na in names:
        fa = pool[na]
        for nb in names:
            if nb <= na: continue  # commutative ops: skip dupes (only for symmetric ops, but ok)
            fb = pool[nb]
            for nc in names:
                fc = pool[nc]
                for op1_name, op1 in OPS.items():
                    for op2_name, op2 in OPS.items():
                        ...
                        for inp, out in examples:
                            ni = b(inp); no = b(out)
                            v = op2(op1(fa(ni), fb(ni)), fc(ni))
                            if (v & 0xFF) != no:
                                ok = False; break
                        if ok:
                            found.append(f'({na} {op1_name} {nb}) {op2_name} {nc}')
