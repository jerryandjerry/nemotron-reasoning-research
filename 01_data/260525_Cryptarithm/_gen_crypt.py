...
_TOK = _Tokenizer.from_file(os.path.join(os.path.dirname(os.path.abspath(__file__)),
                                         '..', '..', '02_train', '260512_huikang_085', 'repo', 'tokenizer.json'))
...
def _undet_verdict(p, op):
    """Verdict for an operator with no resolved family. The discriminator is ONLY whether the symbol
    appears in an EXAMPLE: if it does, the four-family search ran on its rows and failed -> it is an
    EXOTIC operator; if it appears only in the QUERY there is no example to solve it from -> it is a
    QUERY-only operator. (query-only != exotic; never call an in-example operator query-only.)"""
    ol = p.op_map[op]
    return f"{ol} is an exotic operator" if op in p.by_op else f"{ol} is a QUERY-only operator"
...
def prune_candidates(p, op_set, sign_ops, concat_results, om, out, n):
    ...
    trav = ", ".join(
        ...
        else _undet_verdict(p, op)        # an operator that appears ONLY in the query (unseen) — not determined here
        for op in p.op_syms)
...
def front_matter(p):
    ...
    L.append("Each symbol can be a hidden operator or operand. I work out the digit cipher and each operator from the example equations, then apply them to the QUERY and return the answer. Even a symbol that looks like an operator — /, <, >, %, &, |, ^ — can represent an operand here; the only rule is that the third symbol of the lhs is always the operator.")
    L.append("")
    ...
    L.append("6. If the QUERY operator only appears in the QUERY, we cannot solve it from the examples; we call it a QUERY-only operator. If the QUERY operator appears in the examples but cannot be solved, it is outside the system, and we call it an exotic operator. In either case we still resolve it to a concrete answer.")
    L.append("")
    # sign check
    sign_ops = []
    for i, ex in enumerate(p.examples, 1):
...
def op_state_str(p, sign_ops, drop_concat=False):
    ...
    for op in p.op_syms:
        ...
        if fs is None:
            parts.append(_undet_verdict(p, op))
        else:
...
def op_state_after_concat(p, sign_ops, concat_results):
    ...
    for op in p.op_syms:
        ...
        elif op not in p.by_op:   parts.append(_undet_verdict(p, op))
        else:
            excl = {'noisy_subtraction'} if sign_ops else set()
...
def _conc_brace(p, variants, concat_results, A):
    ...
    for op in p.op_syms:
        ...
        else:                              parts.append(_undet_verdict(p, op))
    if A:
...
def emit_answer(p, A, variants, concat_results, mode, op_set, out):
    ...
    if A:
        ...
        a = 10*A[d1]+A[d2] if mode == 'standard' else 10*A[d2]+A[d1]
        b = 10*A[d3]+A[d4] if mode == 'standard' else 10*A[d4]+A[d3]
        out.append(f"Query operator {zl} appears only in the QUERY, so no example fixes it; I will assign it by default.")
        present = []
        ...
        if not present:                                          # no arithmetic operator solved -> guess concat
            ...
            out.append(f"No operator is solved arithmetically, so {zl} is ~concat. Let me use {zl} = a∥b.")
        ...
        ford_str = ", ".join(tg(f) for f in ford)
        free_str = ", ".join(tg(f) for f in free)
        sym_note = f" (the symbol '{zs}' usually means {tg(canon)})" if canon else ""
        present_pairs = [f"{p.op_map[op]} = {tg(cg._OP_FAMILY.get(variants[op]))}" for op in p.op_syms
                         if op in (variants or {}) and cg._OP_FAMILY.get(variants[op]) in ARITH]
        pres = " and ".join(present_pairs)
        intro = f"At least one operator is solved arithmetically, so {zl} is arithmetic too, so {zl} = [{ford_str}]{sym_note}."
        if free:
            out.append(f"{intro} Since {pres}, the distinct-operator rule leaves {zl} = [{free_str}], so in order I assign {zl} = {tg(chosen)}.")
        else:
            out.append(f"{intro} Since {pres}, every arithmetic operation is already used, so I take the first, {zl} = {tg(chosen)}.")
    ...
    ans = d1+d2+d3+d4
    out.append(f"The examples aren't arithmetically solvable, so by default {zl} = a∥b; answer = {ans}")
    return ans
...
def _finalize(L, letters):
    ...
    # never touch the boxed answer (must stay exact for the metric); also protect the literal word
    # ASCII — it can never be puzzle content (S is never a cipher letter, only A-J are assignable), but
    # its 'CII' run would otherwise be spaced to 'ASC I I' whenever C and I are cipher letters.
    return ''.join(part if (part.startswith('\\boxed') or part == 'ASCII') else pat.sub(sp, part)
                   for part in re.split(r'(\\boxed\{[^}]*\}|ASCII)', s))
...
def gen_cot(name):
    ...
    if qop not in p.by_op:                                       # STEP 1: unseen query operator — log it (always)
        zl = p.op_map[qop]
        L.append(f"Note: Query operator {zl} appears only in the QUERY, so it is a QUERY-only operator that we resolve by default.")
        ...
        if RULE1_CONCAT and max(rhs_lens) < 4:                   # STEP 2: rule #1 met — no 4-digit RHS => guess ~concat
            ...
            L.append(f"There is no 4-digit RHS in any example, so by default {zl} is concatenation. I use {zl} = a∥b.")
        ...
        if max(rhs_lens) >= 4:                                       # STEP 2: rule #1 not met — a 4-digit RHS makes concat risky
            ...
            L.append(f"There is a 4-digit RHS in EX{ex4}, so concatenation is risky for {zl}; I'll solve the puzzle and see what happens.")
    ...
    for mode, n in [('standard', 1), ('little_endian', 2)]:
        ...
        if A == 'CONCAT_SHORT':     # query op is concat -> short-circuit straight to the answer
            brace = "{" + ", ".join((f"{p.op_map[op]} = {CDISP[concat_results[op]]}" if op in concat_results else _undet_verdict(p, op)) for op in p.op_syms) + "}"
    ...
    if short_circuit:
        ...
        asg = ", ".join((f"{p.op_map[op]} = {CDISP[concat_results[op]]}" if op in (concat_results or {}) else _undet_verdict(p, op)) for op in p.op_syms)
        ...
        if legal_ns:
            ...
            asg = ", ".join((f"{p.op_map[op]} = {SYMB[variants[op]]}" if variants and op in variants else (f"{p.op_map[op]} = {CDISP[concat_results[op]]}" if op in (concat_results or {}) else _undet_verdict(p, op))) for op in p.op_syms)
        ...
        mode, A, variants, concat_results = win
        L.append(f"Summary: Confirm reading order = {conf_reading(mode)}, " + ", ".join((f"{p.op_map[op]} = {SYMB[variants[op]]}" if variants and op in variants else (f"{p.op_map[op]} = {CDISP[concat_results[op]]}" if op in (concat_results or {}) else _undet_verdict(p, op))) for op in p.op_syms))
    L.append("")
    mode, A, variants, concat_results = win
...
def _fill_missing(A, op_set, ans, missing):
    ...
    head = (f"But digit {digs} doesn't show up in the examples, so I assign its symbol by default."
            if len(guess) == 1 else
            f"But digits {digs} don't show up in the examples, so I assign their symbols by default.")
    return ([head,
