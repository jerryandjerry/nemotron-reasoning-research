...
VARIANT_FN = {
    ...
    'sub_signed_p1': lambda a, b: a - b + 1,
    'sub_signed_m1': lambda a, b: a - b - 1,
    'sub_signed_p2': lambda a, b: a - b + 2,
    'sub_signed_m2': lambda a, b: a - b - 2,
}
...
CORES = {   # variants in FREQUENCY ORDER (per the prior); the solver tries them in this exact order
    'mul':               ['mul', 'mul_p1', 'mul_m1', 'mul_p2', 'mul_m2'],
    'add':               ['add', 'add_p1', 'add_m1', 'add_p2', 'add_m2'],
    'noisy_subtraction': ['absdiff', 'sub_signed', 'neg_absdiff', 'rsub_signed',
...
def op_str(name, a, b):
    ...
    return {
        ...
        'neg_absdiff':  f"-|{a}-{b}|={r}",
        'rsub_signed':  f"{b}-{a}={r}",
        'sub_signed_p1': f"{a}-{b}+1={r}",
        'sub_signed_m1': f"{a}-{b}-1={r}",
        'sub_signed_p2': f"{a}-{b}+2={r}",
        'sub_signed_m2': f"{a}-{b}-2={r}",
    }.get(name, str(r))
...
def parse(path):
    ...
    # Letters are assigned in ORDER OF FIRST APPEARANCE while reading the equations left-to-right
    # (operands then RHS), then the query — a puzzle-grounded order, not an ASCII sort. This forces
    # the assignment to depend on the puzzle text, so it cannot be produced from a memorized symbol pool.
    digit_syms = []
    for ex in examples:
        for ch in (ex.d1, ex.d2, ex.d3, ex.d4, *ex.rhs):
            if ch not in op_set and ch not in digit_syms: digit_syms.append(ch)
...
_OP_FAMILY = {
    'mul': 'noisy_mul', 'mul_p1': 'noisy_mul', 'mul_m1': 'noisy_mul', 'mul_p2': 'noisy_mul', 'mul_m2': 'noisy_mul',
    'add': 'noisy_add', 'add_p1': 'noisy_add', 'add_m1': 'noisy_add', 'add_p2': 'noisy_add', 'add_m2': 'noisy_add',
    'absdiff': 'noisy_subtraction', 'sub_signed': 'noisy_subtraction',
    'neg_absdiff': 'noisy_subtraction', 'rsub_signed': 'noisy_subtraction',
    'sub_signed_p1': 'noisy_subtraction', 'sub_signed_m1': 'noisy_subtraction',
    'sub_signed_p2': 'noisy_subtraction', 'sub_signed_m2': 'noisy_subtraction',
}
