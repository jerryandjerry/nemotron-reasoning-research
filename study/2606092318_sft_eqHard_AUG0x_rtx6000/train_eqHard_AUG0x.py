#!/usr/bin/env python3
"""
Stage 1 SFT on 260609_Crypt3k_eqHardened_AUG0x_FULL.csv (huikang pretok_0408_decoded 5545 +
260608_crypt10k 2693 + 260607_NE_aug 1575 + 260601_new_solver 800 + 260606_new_solver 732;
NO aug-row oversampling — this is the "build base ability" stage).

Diff vs cryptaug_1000 trainer (per feedback_copy_sft_verbatim — verbatim except CSV swap):
  - CSV_PATH      -> /root/autodl-tmp/data/260609_Crypt3k_eqHardened_AUG0x_FULL.csv
                     (md5 6c2ad37022f1e6b91aa4ad5703847c07)
  - _DATA_TAG     -> 'eqHard_AUG0x'

Recipe (byte-identical to #16 / #30): bf16 + CCE + no-tie + live out_proj + manual lm_head LoRA,
...
Expected at startup:
  CSV rows: 11345; rows with oversampling>0: 6473
  Expanded ~7229 -> ~226 steps at bs=32 (~3.7 hrs at ~1 min/step)
"""
...
CSV_PATH   = '/root/autodl-tmp/data/260609_Crypt3k_eqHardened_AUG0x_FULL.csv'  # eqHardened, AUG0x oversampling -> 6473 train -> 7229 expanded -> ~226 steps
...
_DATA_TAG  = 'eqHard_AUG0x'  # Stage 1: eqHardened, no oversampling on aug rows ("build base ability")
