#!/usr/bin/env python3
"""
Curriculum Stage 3 low-init variant: lower-loss Stage 2 adapter -> Stage 3 data with one continuous WSD schedule.

Same training loop/code lineage as Stage 1, except:
  - LOG_FILE/_DATA_TAG/RUN_NAME naming -> curriculum_stage3_lowinit
  - CSV_PATH -> 260614_STAGE3_FULL.csv
  - peak LR -> 1.5e-4
  - schedule -> WSD: 5% warmup, hold until 70%, final 30% linear decay to 0

Run with: MOE_TIE=0 USE_MEM_EFF=0 FRESH_START=1 and
INIT_ADAPTER_FROM=/root/autodl-tmp/stage3_lowinit_adapter_init
"""
...
LOG_FILE = f"/root/autodl-tmp/train_log_curriculum_stage3_lowinit_{'fixtie' if os.environ.get('MOE_TIE', '1') == '1' else 'notie'}{'' if os.environ.get('USE_MEM_EFF', '1') == '1' else '_outproj'}.txt"
...
CSV_PATH   = '/root/autodl-tmp/data/260614_STAGE3_FULL.csv'
...
_DATA_TAG  = 'curriculum_stage3_lowinit'
...
SAVE_STEPS = None  # computed after num_steps per TRAINING_ROUTINE.md
...
RUN_NAME = f'{_ts}_codex_sft_stage3_lowinit_wsd_contS2_rtx6000'
...
# ── Stage 3: load Stage 2 adapter as init (fresh optimizer + scheduler stay) ───────
...
# restored -- Stage 3 trains a clean 1-epoch cycle on the FULL data.
...
if _INIT_ADAPTER_FROM:
    ...
    for _k, _v in _init_state.items():
        ...
        # Kaggle-ready submission.zip stores lm_head LoRA under backbone.lm_head;
        # the fresh PEFT model exposes it at the original lm_head path.
    ...
    print('INIT_ADAPTER_FROM unset; LoRA weights left at random init')
...
_log(f'LR schedule: WSD warmup_steps={WARMUP_STEPS}, stable_until_step={DECAY_START_STEP}, decay_steps={DECAY_STEPS}, peak_lr={LEARNING_RATE}')

def _lr_for_step(step_num):
    # step_num is 1-indexed for the optimizer update being applied.
    if step_num <= WARMUP_STEPS:
        return LEARNING_RATE * step_num / WARMUP_STEPS
    if step_num <= DECAY_START_STEP:
        return LEARNING_RATE
    progress = (step_num - DECAY_START_STEP) / DECAY_STEPS
...
_log(f'LR:           WSD peak {LEARNING_RATE}, warmup_steps={WARMUP_STEPS}, stable_until_step={DECAY_START_STEP}, decay_steps={DECAY_STEPS}')
