...
not by ASCII. The solver reads ONLY question.txt and NEVER the gold answer. Run `python3 _gen_eq.py`
for every puzzle folder beside this file, or `python3 _gen_eq.py <id>` to solve a single one.
"""
import os, re, sys, csv, math, collections, glob

OUT_DIR = os.path.dirname(os.path.abspath(__file__))
# Self-contained: the only thing this solver needs is ./reasoners/ (the candidate-operation
# functions), sitting next to this file. No data files, no CSVs, no gold answers — ever.
sys.path.insert(0, OUT_DIR)                               # resolve `from reasoners...` to the LOCAL ./reasoners/ copy
...
def gen_cot(d):
    ...
    L.append("6. I should never return the answer as \"unknown\". When the final result is "
             "undetermined, I should make a best guess using what I know.")
    ...
    dctstr = lambda vmap: ", ".join((f"{label[op]} = {cexpr(vmap[op], 'a', 'b')}" if vmap.get(op) else (f"{label[op]} = unknown" if (not seen and op == qop) else f"{label[op]} is an exotic operator")) for op in full_order)
    ...
    if seen and variants.get(qop) in ('concatenation', 'reverse concatenation'):
        ...
        L.append(f"Query operator {label[qop]} only appears in QUERY, so I need to guess it.")
        ...
        if not used_fams:
            ...
            L.append(f"No operator here can be solved arithmetically, so I guess the unseen operator "
                     f"{label[qop]} is ~concat. Let me use {label[qop]} = {cexpr(hk, 'a', 'b')}.")
            raw = raw_on(qsa, qsb, hk, ro); final = _rev(raw) if rr else raw
            ans = render_answer(final, fmt, qop)
            emit_answer(hk, raw, final, ans)
            ...
            prior = (f"the symbol '{qop}' usually means {tg(canon)}, so {tg(chosen_fam)} is the pick" if (canon and chosen_fam == canon)
                     else f"{tg(chosen_fam)} comes first in the default order, so it is the pick")
...
# ── parse the solver's OWN boxed answer out of the CoT it just wrote (no gold involved) ──────────
...
def load_folder(folder):                                 # a solver reads ONLY question.txt — never the gold answer
    prompt = open(os.path.join(folder, 'question.txt')).read()
    ex, qn = parse_prompt(prompt)
    return {'prompt': prompt, 'examples': ex, 'question': qn}


def main():
    only = sys.argv[1] if len(sys.argv) > 1 else None    # solve a single puzzle id; otherwise solve all
    ...
    for folder in folders:
        ...
        d = load_folder(folder)                          # question.txt only
        cot, _ = gen_cot(d)                              # SOLVE — blind: examples + query, never the gold
        os.makedirs(os.path.join(folder, 'track'), exist_ok=True)
        open(os.path.join(folder, 'track', 'tree_cot.txt'), 'w').write(cot)   # question.txt -> track/tree_cot.txt
        n += 1
        print(f"equation_numeric_{pid}: answer = {extract_final_answer(cot)}  -> track/tree_cot.txt")
    if not only:
        print(f"\nsolved {n} puzzle(s) under {OUT_DIR}")
