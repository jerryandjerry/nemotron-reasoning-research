#!/usr/bin/env python3
"""
Retrain of run #7 (huikang_lkall_stripped + Kaggle regex, scored 0.84) with:
  - Adapter-only soup saves enabled per routine §2.4B
    (every save_steps + 80/85/90/95/100% of num_steps, no FIFO, all on disk)
  - Resume-from-latest-checkpoint per routine required feature
    (handles PEFT save_pretrained's stripped '.default.' suffix)
  - FIFO=1 for full ckpt per routine §2.4A

Soup recipes built after training (user defaults, 2026-05-18):
  1. soup-last5         = uniform avg of last 5 adapter saves {200, 209, 221, 234, 246}
  2. wise-50-150-246    = 0.5/3 × {50, 150, 246}

Data + config byte-identical to run #7: 260515_huikang_lkall_stripped.csv,
...
CSV_PATH   = '/root/autodl-tmp/data/260515_huikang_lkall_stripped.csv'
TOKENIZER_JSON = '/root/autodl-tmp/data/tokenizer.json'
OUTPUT_DIR = '/root/autodl-tmp/adapter_output_huikang_lkall_stripped_soup'
...
RUN_NAME = f'{_ts}_sft_huikang_lkall_stripped_soup_r32_a32_lr2e4_seq8192_bs32_rtx6000'
