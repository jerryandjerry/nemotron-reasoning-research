#!/usr/bin/env python3
"""
Huikang 0.85 config with pre-tokenized data, but load_in_4bit=True (QLoRA).

Same as the 0.84 reproduction except base model is 4-bit quantized.
All other config identical: CCE, MoE tying, r32, lr 2e-4, bs32, etc.
"""

import os, sys, time, json, zipfile, gc, subprocess, types, math, logging, random as _random
...
MICRO_BATCH_SIZE = 16
# Stress test results:
# bs=8  micro=8  → 60GB peak PASS
# bs=16 micro=16 → 86GB peak PASS
# bs=32 micro=16 → testing...
LEARNING_RATE = 2e-4
NUM_STEPS    = 1000
...
OUTPUT_DIR = '/root/autodl-tmp/adapter_output_qlora4bit_pretok'
...
RUN_NAME = f'{_ts}_sft_qlora4bit_pretok7830_r32_a32_lr2e4_seq8192_bs32_rtx6000'
...
for sid in ordered_ids:
    ...
    examples.append({
        ...
        'content_tail': '',  # pre-tokenized, no text to decode yet
    })
...
if STRESS_TEST:
    ...
    _log(f'Stress test: {len(examples)} longest examples')

# ── Load base model (4-bit quantized) ───────────────────────────────
_log('\n=== Loading 4-bit model via Unsloth ===')
...
_log(f'Model loaded in {time.time() - t_load:.1f}s')
...
_log(f'VRAM after load: smi={_smi()}M')

# Decode content_tail now that tokenizer is loaded
for e in examples:
    toks = e['tokens'] + [e['targets'][-1]] if e['targets'] else e['tokens']
    e['content_tail'] = tokenizer.decode(toks[-20:], skip_special_tokens=False)[-20:]
...
if nemotron_mod is not None:
    _log(f'is_fast_path_available was: {nemotron_mod.is_fast_path_available}')
    nemotron_mod.is_fast_path_available = True
    _log('Patched is_fast_path_available = True')
...
if not isinstance(_lm_head, LoraLinear):
    ...
    _log('Manually added LoRA to lm_head')
else:
    _log('lm_head already has LoRA')

# ── Cast LoRA params to fp32 ────────────────────────────────────────
for name, param in model.named_parameters():
    if '.lora_' in name:
        param.data = param.data.to(torch.float32)
_log('Cast LoRA params to fp32')

trainable = sum(p.numel() for p in model.parameters() if p.requires_grad)
total = sum(p.numel() for p in model.parameters())
_log(f'Model: {trainable:,} trainable / {total:,} total')
...
def _patched_causal_forward(input_ids=None, attention_mask=None, labels=None, **kwargs):
    ...
    if labels is not None:
        # CCE backward requires bf16/fp16 embeddings — cast explicitly for 4-bit models
        per_token_ce = linear_cross_entropy(hidden_states.to(torch.bfloat16), lm_weight.to(torch.bfloat16), labels, reduction='none')
...
_log('Patched CausalLM.forward with CCE')
...
if MOE_TIE_WEIGHTS:
    ...
    _log(f'MoE weight tying: {len(moe_tied_params)} params')
    _tie_param_init()
else:
    def _tie_grads():
        pass
...
def _stratified_batches(examples, batch_size, rng):
    ...
    by_cat = {}
    # Pre-tokenized doesn't have category in dict, use problem_id prefix as proxy
    # Actually we can load category from index.jsonl
    for i, ex in enumerate(examples):
        by_cat.setdefault(ex.get('category', 'unknown'), []).append(i)
...
# Load categories from index.jsonl
_cat_map = {}
with open(TRAIN_ORDER_PATH) as f:
    for line in f:
        rec = json.loads(line)
        _cat_map[rec['problem_id']] = rec.get('category', 'unknown')
for e in examples:
    e['category'] = _cat_map.get(e['problem_id'], 'unknown')
...
_log(f'Batching: {"stratified" if batches else "sequential (stress test)"}, {num_steps} steps')
...
for step_idx in range(num_steps):
    ...
    # Checkpoint saving (full: adapter + optimizer + rng + tokenizer + args)
    if step % SAVE_STEPS == 0 or step == num_steps:
        ...
        torch.save({
            ...
            'training_args': {
                ...
                'moe_tie_weights': MOE_TIE_WEIGHTS, 'load_in_4bit': True,
            },
...
# ── Summary ─────────────────────────────────────────────────────────
_log('\n' + '='*60)
_log('TRAINING SUMMARY — huikang config + QLoRA 4-bit + pretokenized')
_log('='*60)
_log(f'Method:       QLoRA 4-bit + CCE + MoE tying')
