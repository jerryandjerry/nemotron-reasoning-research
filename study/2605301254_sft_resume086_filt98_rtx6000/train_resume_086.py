#!/usr/bin/env python3
"""
Resume training of the Sub #16 0.86 adapter on filtered cryptnumeq data.

Diff vs cryptnumeq trainer:
  - CSV_PATH       -> 260530_huikang_NumericEq_5cat15_crg3x.csv
                      The 5 "avoid-forget" cats (bit_manipulation, cipher, gravity,
                      unit_conversion, numeral) sampled at 15%, oversampling=1.
                      Those rows have byte-identical CoTs in #16's training data
                      (verified: 1354+1576+975+650+990 IDs all match with same CoT),
                      so heavy re-exposure risks overfit / forgetting other capabilities.
                      The 4 "new-trace" cats (equation_numeric_deduce/guess,
                      cryptarithm_deduce/guess) kept FULL — their CoTs are new-solver
                      style, never seen by #16. cryptarithm_guess bumped to 3x
                      (was 2x in source) to boost coverage (small cat, 22 unique).
  - _DATA_TAG      -> 'resume086_filt98'  (folder/output naming retained)
  - LEARNING_RATE  -> 5e-5 (gentle resume from converged #16 weights)
  - ADAPTER_INIT_PATH env: if set AND OUTPUT_DIR has no checkpoints, load that
    adapter as the LoRA init (with lm_head inverse-rename).

Recipe (byte-identical to #16): bf16 + CCE + no-tie + live out_proj +
manual lm_head LoRA, r32 a32, batch 32 (micro 4 ga 8), seq 8192,
stratified seed 42, soup adapters at 20/60/80/85/90/95/100% of num_steps.

Expected at startup:
  CSV rows (unique): 1715  (md5 5551b00711fae8e8d21177d77e9be52c)
  Expanded 2201 -> 69 steps (1 epoch at bs=32)
"""
...
CSV_PATH   = '/root/autodl-tmp/data/260530_huikang_NumericEq_5cat15_crg3x.csv'  # 5 avoid-forget cats at 15% (oversampling=1); 4 new-trace cats kept full (cr_g bumped 2x->3x); 1715 unique -> 2201 expanded -> 69 steps
...
_DATA_TAG  = 'resume086_filt98'  # distinguishes this run's outputs (resume from #16 0.86, filtered to >98%-acc cats at 10%)
...
try:
    ...
    swanlab.init(
        ...
        config={
            ...
            'data': 'resume #16 on filt98: cryptnumeq with cipher/unit_conv/gravity/numeral at 10% (2658 unique -> 112 steps)',
