#!/usr/bin/env python3
"""
EPOCH 2 of a 2-epoch run paused after epoch 1 for a Kaggle submission.
Resumes the SAME OUTPUT_DIR as epoch 1, picks up at step 69 (from checkpoint-69_*),
runs to step 138.

Diff vs epoch-1 trainer:
  - CSV_PATH       -> 260530_huikang_NumericEq_5cat15_crg3x_ep2.csv
                      A different 15% sample of the 5 avoid-forget cats (NO overlap
                      with epoch 1's 15%) + identical 4 new-trace cats (cr_g 3x).
  - _DATA_TAG      -> 'resume086_filt98' (SAME as epoch 1 — same OUTPUT_DIR for
                      true checkpoint resume; trainer finds checkpoint-69_* in
                      OUTPUT_DIR and resumes from it with optimizer state).
  - TOTAL_STEPS env (NEW) -> 138 = epoch_1 num_steps (69) + epoch_2 num_steps (69).
                      Treats this as the second half of a 138-step plan:
                        - LR schedule lr=5e-5*(1-step/138): step 69 -> lr=2.5e-5;
                          step 138 -> lr=0 (linear decay across epoch 2).
                        - Batches list padded with placeholders at the front to
                          length 138; loop range(_resume_step=69, 138) accesses
                          only batches[69..137] = the real ep2 batches.
                      ADAPTER_INIT_PATH is NOT used (resume comes from checkpoint).
  - LEARNING_RATE  -> 5e-5 (unchanged; schedule applied with num_steps=138)

Recipe (byte-identical to #16): bf16 + CCE + no-tie + live out_proj +
manual lm_head LoRA, r32 a32, batch 32 (micro 4 ga 8), seq 8192,
stratified seed 42, soup adapters at 20/60/80/85/90/95/100% of num_steps=138
  -> {28, 83, 110, 117, 124, 131, 138}. Only steps > _resume_step (69) fire,
  so epoch 2 saves at {83, 110, 117, 124, 131, 138} = 6 of 7 (step 28 already
  passed; epoch 1's own 20% save at step 14 was deleted to free disk).

Expected at startup:
  CSV rows (unique): 1715  (md5 525ed18f4617cfe2f54797461f268180)
  Expanded 2201 -> 69 batches, padded to 138 via TOTAL_STEPS=138
  Resuming from checkpoint-69_loss0.0716_lr7.25e-07 (step 69)
  Loaded adapter + optimizer state (warm AdamW from epoch 1)
"""
...
LOG_FILE = f"/root/autodl-tmp/train_log_newdata_{'fixtie' if os.environ.get('MOE_TIE', '1') == '1' else 'notie'}{'' if os.environ.get('USE_MEM_EFF', '1') == '1' else '_outproj'}.txt"
...
CSV_PATH   = '/root/autodl-tmp/data/260530_huikang_NumericEq_5cat15_crg3x_ep2.csv'  # EP2: different 15% on 5 avoid-forget cats (no overlap w/ ep1), eq+crypt unchanged; 1715 unique -> 2201 expanded -> 69 steps
...
_DATA_TAG  = 'resume086_filt98'  # SAME tag as epoch 1 -> same OUTPUT_DIR -> trainer finds checkpoint-69_* and resumes
...
SAVE_STEPS = None  # computed after num_steps below: max(25, round(num_steps/5/25)*25). For 112 steps -> 25.

STRESS_TEST = bool(os.environ.get('STRESS_TEST'))
FRESH_START = bool(os.environ.get('FRESH_START'))
# If set AND OUTPUT_DIR has no checkpoint dirs, load this external adapter
# (e.g. Sub #16's unzipped 0.86 adapter dir) as the LoRA initialization.
ADAPTER_INIT_PATH = os.environ.get('ADAPTER_INIT_PATH', '')
# Override num_steps after batches are built. Used for multi-epoch continuation:
# pad batches list at the front so the loop's range(_resume_step, TOTAL_STEPS)
# accesses only the new real batches. LR schedule uses TOTAL_STEPS as denominator
# so step N gives lr = LEARNING_RATE * (1 - N/TOTAL_STEPS).
TOTAL_STEPS_OVERRIDE = int(os.environ.get('TOTAL_STEPS', '0') or '0')
...
RUN_NAME = f'{_ts}_sft_{_DATA_TAG}_{_MOE_MODE}{_MEM_SUFFIX}_r32_a32_lr5e5_seq8192_bs32_rtx6000'
...
_log(f'Batching: stratified, {num_steps} steps (from CSV)')

# ── Multi-epoch continuation: override num_steps + pad batches at front ──
# When resuming a paused multi-epoch run, set TOTAL_STEPS to the full plan's
# step count. Batches list is padded with empty placeholders at index 0..pad-1
# so the loop's range(_resume_step, TOTAL_STEPS) only accesses real batches.
if not STRESS_TEST and TOTAL_STEPS_OVERRIDE > 0:
    if TOTAL_STEPS_OVERRIDE < num_steps:
        raise ValueError(f'TOTAL_STEPS={TOTAL_STEPS_OVERRIDE} < computed num_steps={num_steps}')
    pad_count = TOTAL_STEPS_OVERRIDE - num_steps
    if batches is not None and pad_count > 0:
        batches = [[] for _ in range(pad_count)] + batches  # placeholders go at the front
    num_steps = TOTAL_STEPS_OVERRIDE
    _log(f'TOTAL_STEPS override: padded {pad_count} placeholders at front, num_steps={num_steps}')

# Per routine §2.4: max(25, round(num_steps/5/25)*25). 112 -> 25; 246 -> 50.
SAVE_STEPS = max(25, round(num_steps / 5 / 25) * 25)
_log(f'SAVE_STEPS (routine formula): {SAVE_STEPS}')
...
try:
    ...
    swanlab.init(
        ...
        config={
            ...
            'data': 'EP2 resume from ep1 adapter: 5cat15_crg3x_ep2 (different 15% on avoid-forget, same eq+crypt as ep1; 1715 unique -> 69 steps)',
            'adapter_init_path': ADAPTER_INIT_PATH or None,
...
if not FRESH_START:
    ...
    elif ADAPTER_INIT_PATH:
        # First-launch resume from EXTERNAL adapter (e.g. Sub #16's 0.86 weights).
        # No optimizer state available -> fresh AdamW, step 0.
        assert os.path.isdir(ADAPTER_INIT_PATH), \
            f'ADAPTER_INIT_PATH set but directory not found: {ADAPTER_INIT_PATH}'
        _log(f'External adapter init: {ADAPTER_INIT_PATH}')
        _adapter_path = os.path.join(ADAPTER_INIT_PATH, 'adapter_model.safetensors')
        _adapter_state = _load_file(_adapter_path)
        # #16 saved its adapter with lm_head keys RENAMED for Kaggle compatibility:
        #   base_model.model.lm_head.* -> base_model.model.backbone.lm_head.*
        # Invert that here so the keys match our freshly-built PEFT model's lm_head LoRA.
        _restored_state = {
            k.replace('base_model.model.backbone.lm_head.', 'base_model.model.lm_head.'): v
            for k, v in _adapter_state.items()
        }
        # PEFT save_pretrained strips '.default.' from adapter keys; restore for state_dict load
        _renamed_state = {}
        for _k, _v in _restored_state.items():
            _nk = _k
            for _sfx in ('lora_A.weight', 'lora_B.weight',
                         'lora_embedding_A.weight', 'lora_embedding_B.weight'):
                if _nk.endswith('.' + _sfx):
            ...
            _renamed_state[_nk] = _v
        _missing, _unexpected = model.load_state_dict(_renamed_state, strict=False)
        _log(f'  Loaded external adapter: {len(_renamed_state)} tensors '
             f'(missing={len(_missing)}, unexpected={len(_unexpected)})')
        if _unexpected:
            _log(f'  Unexpected keys (first 5): {_unexpected[:5]}')
        if _missing:
            _lora_missing = [m for m in _missing if 'lora_' in m]
            _log(f'  Missing keys: {len(_missing)} total, {len(_lora_missing)} LoRA-related '
                 f'(rest are frozen base params, expected). LoRA missing sample: {_lora_missing[:3]}')
        # Sanity: confirm out_proj LoRA B is non-zero (matches #16's live out_proj signature)
        _outproj_B = [(n, p) for n, p in model.named_parameters()
                      if 'out_proj.lora_B' in n and 'default' in n]
        if _outproj_B:
            import torch as _t
            _norms = [_t.linalg.vector_norm(p.data).item() for _, p in _outproj_B[:5]]
            _log(f'  out_proj.lora_B norms (first 5): {[round(n, 4) for n in _norms]} '
                 f'(should be > 0; #16 reference ~0.7)')
        _log('  Note: training from step 0 with fresh AdamW (no optimizer state for external adapter init)')
    else:
        _log('No checkpoint to resume from and ADAPTER_INIT_PATH not set, starting fresh')
else:
...
# ── Summary ─────────────────────────────────────────────────────────
_log('\n' + '='*60)
_log('TRAINING SUMMARY — resume of Sub #16 (0.86) on filtered cryptnumeq data')
_log('='*60)
_log(f'Method:       LoRA bf16 + CCE + no-tie + live out_proj (resume from #16 adapter)')
_log(f'Adapter init: {ADAPTER_INIT_PATH or "(fresh)"}')
