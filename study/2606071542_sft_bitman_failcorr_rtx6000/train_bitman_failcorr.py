#!/usr/bin/env python3
"""
Fresh SFT on 260516_bit_manipulation_failcorr.csv (#16's golden_stripped base with per-bit ✓/✗
verification block inserted before "Selected" on 1,280 of 1,354 bit_manipulation rows; non-bit_man
rows pass through unchanged from huikang_golden_stripped).

Diff vs the 06-04 cryptarithm_gtTrue trainer (per feedback_copy_sft_verbatim — verbatim except CSV swap):
  - CSV_PATH      -> /root/autodl-tmp/data/260516_bit_manipulation_failcorr.csv (md5 a9c2349c3cc88eddeebbc11709a443fa)
  - _DATA_TAG     -> 'bitman_failcorr'
...
Recipe (byte-identical to #16 / cryptnumeq): bf16 + CCE + no-tie + live out_proj +
...
Expected at startup (verify against data audit before launching real training):
  CSV rows (unique): 6905
  Expanded ~7847 -> ~245 steps at bs=32 (~4 hrs at ~1 min/step)
"""
...
CSV_PATH   = '/root/autodl-tmp/data/260516_bit_manipulation_failcorr.csv'  # #16 base + per-bit verification on 1280 bit_man rows -> 6905 unique -> 7847 expanded -> ~245 steps
...
_DATA_TAG  = 'bitman_failcorr'  # huikang_golden_stripped base + per-bit verification on 1280 bit_man rows
