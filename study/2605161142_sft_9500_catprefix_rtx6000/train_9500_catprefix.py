#!/usr/bin/env python3
"""
Huikang 0.84 setup (run #7) + ALL 9,500 official rows, answer-only CoTs with
category prefix.

Difference vs run #7 (2605152212_sft_huikang_lkall_stripped_kaggleregex_rtx6000):
- Data: 260516_9500_answer_only_catprefix.csv — 9,500 rows (full official set),
  solver_cot is the short answer-only boilerplate ("I will now return the answer
  in \\boxed{}...") with a category-identifying prefix prepended. No oversampling
  column → 1× each → ~297 steps.

Hypothesis: model can learn the task from (prompt + category prefix + answer)
without lengthy CoT — testing whether the prefix alone is enough of a "think"
hint to commit to the right approach.

Everything else identical to run #7: bf16 + CCE + MoE tying, r32 a32,
lr 2e-4 linear decay over actual num_steps, AdamW beta2=0.95, bs=32 micro=4,
9 targets + lm_head, seq=8192, stratified batching seed=42, Kaggle-metric
\\boxed extractor.
"""
...
CSV_PATH   = '/root/autodl-tmp/data/260516_9500_answer_only_catprefix.csv'
...
RUN_NAME = f'{_ts}_sft_9500_catprefix_r32_a32_lr2e4_seq8192_bs32_rtx6000'
...
# ── Adapter-only soup save schedule (no FIFO, kept all on disk) ───────
# Every SAVE_STEPS plus 80%/85%/90%/95% of num_steps. Final step (100%)
# already covered by the FIFO full checkpoint.
...
ADAPTER_SAVE_STEPS = [s for s in ADAPTER_SAVE_STEPS if s != num_steps and s > 0]
...
try:
    ...
    swanlab.init(
        ...
        config={
            ...
            'data': 'huikang_non_cryptarithm (6106) + lkevincc_all_cryptarithm (800: 65 swapped + 735, ✓ stripped, 45076dc9 fixed)',
...
if not FRESH_START:
    ...
    if _ckpt_dirs:
        ...
        if _missing:
            # Sanity: most missing should be frozen base-model params
        ...
        # Re-tie MoE params after weight load (they were tied at init; loaded values preserve tying)
    else:
...
# ── Zip adapter-only checkpoints (for LoRA soup) ─────────────────────
_log('\n=== Zipping adapter-only checkpoints for soup ===')
adapter_dirs = sorted([d for d in os.listdir(OUTPUT_DIR) if d.startswith('adapter-')],
                      key=lambda x: int(x.split('-')[1].split('_')[0]))
for ad in adapter_dirs:
    ad_zip_path = os.path.join(os.path.dirname(OUTPUT_DIR), f'{ad}.zip')
    ad_full = os.path.join(OUTPUT_DIR, ad)
    with zipfile.ZipFile(ad_zip_path, 'w', zipfile.ZIP_DEFLATED) as zf:
        for root, dirs, files in os.walk(ad_full):
            for f in files:
                fp = os.path.join(root, f)
                zf.write(fp, arcname=os.path.relpath(fp, ad_full))
    _log(f'  Adapter zip: {ad_zip_path} ({os.path.getsize(ad_zip_path)/1024/1024:.1f} MB)')

# ── Summary ─────────────────────────────────────────────────────────
_log('\n' + '='*60)
_log('TRAINING SUMMARY — huikang non-cryptarithm + ALL lkevincc cryptarithm + Kaggle regex')
_log('='*60)
