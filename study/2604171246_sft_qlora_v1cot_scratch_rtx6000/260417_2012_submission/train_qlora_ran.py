#!/usr/bin/env python3
"""
QLoRA training on AutoDL RTX PRO 6000 — v1 CoT from scratch.

Train from scratch (no pretrained adapter) on v1 CoT data, all 7 categories.
Per-category sampling: cap 800 per category (take all if < 800).
Config: r=32, alpha=16 (scale 0.5), dropout=0.05, lr=1e-4, 1 epoch, seq=8000.
"""
...
# v1 CoT data — all 7 categories, cap 800 per category
DATA_DIR = '/root/autodl-tmp/data/260416_train_cot_v1'
CATEGORY_CAP = 800
...
RUN_NAME = f'{_ts}_sft_qlora_v1cot_scratch_r32_a16_lr1e4_seq8000_ep1_bs4_rtx6000'

MODEL_PATH = '/root/autodl-tmp/Nemotron-3-Nano-30B-A3B'
OUTPUT_DIR = '/root/autodl-tmp/adapter_output_v1cot_scratch'
...
for fpath in sorted(os.listdir(DATA_DIR)):
    if not fpath.endswith('.csv'):
        continue
    df = pl.read_csv(os.path.join(DATA_DIR, fpath)).with_columns(pl.col('answer').cast(pl.Utf8))
    print(f'  {fpath}: {len(df)} rows')
    dfs.append(df)
all_df = pl.concat(dfs)
print(f'Total before sampling: {len(all_df)}')

# Per-category sampling with cap
sampled_parts = []
for cat in all_df['category'].unique().sort().to_list():
    subset = all_df.filter(pl.col('category') == cat)
    sampled = subset if len(subset) <= CATEGORY_CAP else subset.sample(n=CATEGORY_CAP, seed=SEED)
    print(f'  {cat}: {len(subset)} -> {len(sampled)}')
...
training_args = SFTConfig(
    ...
    save_steps                  = 225,  # ~1156 total steps / 5 ≈ 231, rounded to 225
    save_total_limit            = 2,
...
# ── Summary ─────────────────────────────────────────────────────────
print('\n' + '='*60)
print('TRAINING SUMMARY — QLoRA Unsloth + v1 CoT from scratch')
print('='*60)
