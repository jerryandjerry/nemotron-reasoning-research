#!/usr/bin/env python3
"""
Reproduction of run #6 (huikang_golden_stripped + Kaggle regex, scored 0.84) with soups.
Same script as run #10 EXCEPT CSV_PATH → 260514_huikang_golden_stripped.csv.

Expected at startup (must match run #6 exactly to confirm byte-identical data):
...
bf16 + CCE + MoE tying, r32 a32, lr 2e-4 linear decay, bs=32 (micro=4, ga=8),
...
LOG_FILE = f"/root/autodl-tmp/train_log_moe_{'fixtie' if os.environ.get('MOE_TIE', '1') == '1' else 'notie'}{'' if os.environ.get('USE_MEM_EFF', '1') == '1' else '_outproj'}.txt"
...
CSV_PATH   = '/root/autodl-tmp/data/260523_huikang_newsolver_goldonly.csv'
...
OUTPUT_DIR = f'/root/autodl-tmp/adapter_output_moe_{_MOE_MODE}{_MEM_SUFFIX}'
ZIP_PATH   = f'/root/autodl-tmp/submission_moe_{_MOE_MODE}{_MEM_SUFFIX}.zip'
...
RUN_NAME = f'{_ts}_sft_moe_{_MOE_MODE}{_MEM_SUFFIX}_r32_a32_lr2e4_seq8192_bs32_rtx6000'
