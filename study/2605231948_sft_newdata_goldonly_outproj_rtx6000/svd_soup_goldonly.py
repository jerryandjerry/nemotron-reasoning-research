...
Recipes are picked from on-disk adapters by % of the final step (last5 + wise-20/60/100%)."""
...
# Recipes are picked from the adapters actually on disk, by % of the final step,
# so they track the train script's percentage save schedule for any num_steps.
_steps = sorted(step_to_dir.keys())
_final = _steps[-1]
def _nearest(frac):
    return min(_steps, key=lambda s: abs(s - frac * _final))
_last5 = _steps[-5:]
_wise = sorted({_nearest(0.20), _nearest(0.60), _final})
SOUPS = [
    ('svd-soup-last5', _last5),
    ('svd-wise-' + '-'.join(map(str, _wise)), _wise),
]
print(f'Recipes: last5={_last5}  wise={_wise}')
