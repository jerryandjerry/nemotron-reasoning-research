#!/usr/bin/env python3
"""
QLoRA 4-bit Training on AutoDL 2x RTX 4090 (2x 24GB VRAM)
Same recipe as 0.72 LoRA. Model split across 2 GPUs: 26+26 layers.
"""
...
# ── Load pre-quantized model across 2 GPUs ──────────────────────────
print('\n=== Loading pre-quantized NF4 model (2x GPU split) ===')
t_load = time.time()

# Manual device_map: 26 layers per GPU (from model card)
device_map = {"backbone.embeddings": 0, "backbone.norm_f": 1, "lm_head": 1}
for i in range(26):
    device_map[f"backbone.layers.{i}"] = 0
for i in range(26, 52):
    device_map[f"backbone.layers.{i}"] = 1
...
for i in range(torch.cuda.device_count()):
    print(f'GPU {i} VRAM: {torch.cuda.memory_allocated(i)/1024**3:.1f} GB')
...
for i in range(torch.cuda.device_count()):
    print(f'GPU {i} VRAM after LoRA: {torch.cuda.memory_allocated(i)/1024**3:.1f} GB')
...
class LiveProgressCallback(TrainerCallback):
    ...
    def on_step_end(self, args, state, control, **kwargs):
        ...
        self.pbar.set_postfix_str(f'{loss_str}  gpu0={vram0:.1f}G  gpu1={vram1:.1f}G  elapsed={elapsed/60:.1f}m  eta={eta/60:.1f}m')
        self.pbar.update(1)
        sys.stdout.flush()
...
print('='*60)
print(f'Method:      QLoRA 4-bit (BNB NF4, pre-quantized, 2x GPU)')
