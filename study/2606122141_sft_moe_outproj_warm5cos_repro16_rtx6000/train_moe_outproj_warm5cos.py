#!/usr/bin/env python3
"""
Reproduction of run #16 (first TRAINED 0.86: no-tie + live out_proj) with ONE change:
the LR schedule is now 5% linear WARMUP then COSINE decay to 0 over the remaining 95%
(was: linear decay 2e-4 -> 0, no warmup). Everything else byte-identical to
train_moe_outproj.py (run #16). Single-variable schedule-shape experiment.

Run with the run-#16 env: MOE_TIE=0 USE_MEM_EFF=0 FRESH_START=1 (no-tie, live out_proj,
random LoRA init, 246 steps, huikang_golden_stripped data).

Expected at startup (must match run #16 exactly to confirm byte-identical data):
  CSV rows (unique): 6906
  Loaded 7849 examples, 27,989,496 tokens (unmasked=26,713,988)

Soup recipes built after training:
  1. soup-last5         = uniform avg of last 5 adapter saves {200, 209, 221, 234, 246}
  2. wise-50-150-246    = 0.5/3 × {50, 150, 246}

bf16 + CCE, r32 a32, PEAK lr 2e-4, 5% warmup + 95% cosine-to-0, bs=32 (micro=4, ga=8),
seq=8192, stratified batching seed=42, Kaggle regex \\boxed extractor.
"""
...
LOG_FILE = f"/root/autodl-tmp/train_log_moe_{'fixtie' if os.environ.get('MOE_TIE', '1') == '1' else 'notie'}{'' if os.environ.get('USE_MEM_EFF', '1') == '1' else '_outproj'}_warm5cos.txt"
...
OUTPUT_DIR = f'/root/autodl-tmp/adapter_output_moe_{_MOE_MODE}{_MEM_SUFFIX}_warm5cos'
ZIP_PATH   = f'/root/autodl-tmp/submission_moe_{_MOE_MODE}{_MEM_SUFFIX}_warm5cos.zip'
...
RUN_NAME = f'{_ts}_sft_moe_{_MOE_MODE}{_MEM_SUFFIX}_r32_a32_lr2e4_warm5cos_seq8192_bs32_rtx6000'
...
# Per routine §2.4B: every SAVE_STEPS + 80/85/90/95/100% of num_steps.
# Final step (100%) is included here so build_soups.py has a uniform location.
ADAPTER_SAVE_STEPS = sorted({
    *range(SAVE_STEPS, num_steps, SAVE_STEPS),
    round(num_steps * 0.80),
    round(num_steps * 0.85),
    round(num_steps * 0.90),
    round(num_steps * 0.95),
...
for step_idx in range(_resume_step, num_steps):
    ...
    # Routine §1.6 out_proj-alive gate (STRESS_TEST only; no effect on real training).
    if STRESS_TEST and step == _resume_step:
        _op = sum(p.grad.norm().item() for n, p in model.named_parameters()
                  if 'out_proj.lora_B' in n and p.grad is not None)
        _log(f'out_proj.lora_B grad norm sum (stress step 1): {_op:.4f} {"> 0 OK" if _op > 1e-6 else "<= 0 DEAD"}')
        assert _op > 1e-6, 'out_proj LoRA is DEAD — fused Mamba kernel folded it; check USE_MEM_EFF=0'
    ...
    if optimizer is None:
        ...
        if _resume_optimizer_state is not None:
            ...
            _resume_optimizer_state = None
    # LR schedule: 5% linear warmup -> cosine decay to 0 over remaining 95% (HF get_cosine_schedule_with_warmup semantics, num_cycles=0.5).
    # `step` is the 0-indexed step about to be taken. Warmup hits peak at step=_warmup_steps (first cosine step); cosine reaches ~0 at the final step.
    _warmup_steps = max(1, round(num_steps * 0.05))
    if step < _warmup_steps:
        lr = LEARNING_RATE * float(step) / float(_warmup_steps)
    else:
        _cos_progress = float(step - _warmup_steps) / float(max(1, num_steps - _warmup_steps))
...
_log(f'LR:           peak {LEARNING_RATE}, 5% warmup + cosine decay to 0 over remaining 95%')
