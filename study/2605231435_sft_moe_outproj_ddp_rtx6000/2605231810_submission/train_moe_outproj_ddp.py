#!/usr/bin/env python3
"""
DDP 2-GPU replication of run #16 (2605222039_sft_moe_outproj, scored 0.86):
no-tie + LIVE out_proj. Training config + data are BYTE-IDENTICAL to run #16;
ONLY the parallelization changes.
...
- r32/a32/dropout0, lr 2e-4->0 linear, bs 32 (micro 4), seq 8192, 246 steps,
  stratified batching seed 42, CCE + manual lm_head LoRA, Kaggle rfind extractor.
...
Launch: MOE_TIE=0 USE_MEM_EFF=0 torchrun --nproc_per_node=2 train_moe_outproj_ddp.py
"""
...
OUTPUT_DIR = f'/root/autodl-tmp/adapter_output_moe_{_MOE_MODE}{_MEM_SUFFIX}_ddp'
ZIP_PATH   = f'/root/autodl-tmp/submission_moe_{_MOE_MODE}{_MEM_SUFFIX}_ddp.zip'
...
RUN_NAME = f'{_ts}_sft_moe_{_MOE_MODE}{_MEM_SUFFIX}_ddp2gpu_r32_a32_lr2e4_seq8192_bs32_rtx6000'
...
for step_idx in range(_resume_step, num_steps):
    ...
    if optimizer is None:
        ...
        if _resume_optimizer_state is not None:
            ...
            _resume_optimizer_state = None
    lr = LEARNING_RATE * (1 - step / LR_DENOM)
    ...
    if IS_MAIN and step in ADAPTER_SAVE_STEPS:
