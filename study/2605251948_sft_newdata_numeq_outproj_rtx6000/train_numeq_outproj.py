...
CSV_PATH   = '/root/autodl-tmp/data/260525_huikang_NumericEq.csv'  # NumericEq: new-solver CoTs for the numeric-equation categories
...
_DATA_TAG  = 'newdata_numeq'  # distinguishes this run's outputs from the 0.86 / newsolver / goldonly / gt2x runs
...
# Endpoint-only run (no soup requested) -> no soup-adapter saves (routine §2.4B: only when building soups).
ADAPTER_SAVE_STEPS = []
_log(f'Adapter-only soup save steps: {ADAPTER_SAVE_STEPS} (endpoint-only, no soup)')
