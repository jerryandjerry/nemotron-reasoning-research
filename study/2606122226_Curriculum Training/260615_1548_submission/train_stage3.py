#!/usr/bin/env python3
"""
Curriculum Stage 3 — continual-FT from Stage 2 LoRA on the FULL augmented data
(all original + all aug1 + all aug2). SAME proven recipe; USER override = 1 EPOCH only
(recipe doc said 2 epochs; user wants 1). Code byte-identical to Stage 1 except docstring + CSV/naming.

USER CONFIG (Stage 3): NO warmup, NO WSD, NO cosine — keep the favorite linear decay
2e-4 -> 0 with the original peak LR; 1 epoch only (= one pass over the data).

Run with: MOE_TIE=0 USE_MEM_EFF=0 FRESH_START=1 and
  INIT_ADAPTER_FROM=/root/autodl-tmp/adapter_output_curriculum_stage2_notie_outproj/adapter-431_loss0.0051_lr4.64e-07
  -> Stage 3 loads Stage 2 trained LoRA as init (continual-FT), fresh optimizer/scheduler, no-tie, live out_proj.

Data note: 260614_STAGE3_FULL.csv; oversampling=0 rows dropped, rest expanded.
Expected at startup:
  CSV rows: 19961; rows with oversampling>=1: 19095 (all GT-match=True)
  Expanded ~19985 -> ~625 steps at bs=32 (1 epoch, ~10.4 hrs at ~1 min/step)

Soup adapter saves kept verbatim (cheap insurance, [[feedback_always_save_soup_adapters]]);
resumable checkpoint required.
"""
...
CSV_PATH   = '/root/autodl-tmp/data/260614_STAGE3_FULL.csv'  # curriculum Stage 3: full aug pool -> 19095 active -> ~19985 expanded -> ~625 steps (1 epoch)
...
_DATA_TAG  = 'curriculum_stage3'  # Stage 3: full augmented pool, 1 epoch, continual-FT from Stage 2 adapter
