#!/usr/bin/env python3
"""
Curriculum Stage 1 — proven 0.86/0.87 recipe on the curriculum's original-only data.
Code byte-identical to the 0.87 run (2606111516 train_crypt10k_FULL_ep2_contS3.py) EXCEPT:
  - docstring, LOG_FILE/_DATA_TAG naming -> curriculum_stage1
  - CSV_PATH       -> 260614_STAGE1_FULL.csv
  - clip_grad_norm max_norm 1e9 -> 1.0  (routine required feature; inert on normal steps,
    spike insurance — see Sub #36 0.79 collapse from an unclipped early high-LR spike)

Run with: MOE_TIE=0 USE_MEM_EFF=0 FRESH_START=1 and INIT_ADAPTER_FROM UNSET
  -> Stage 1 starts from RANDOM LoRA init (no continual-FT), no-tie, live out_proj.
Schedule unchanged from 0.87 run: linear decay 2e-4 -> 0, NO warmup, NO WSD, NO cosine.

Data note: 260614_STAGE1_FULL.csv marks the aug rows with oversampling=0, so the
n_repeat=int(oversampling) expansion DROPS them -> Stage 1 = original-only.
Expected at startup:
  CSV rows: 15102; rows with oversampling>=1: 6450
  Expanded ~7340 -> ~230 steps at bs=32 (~3.8 hrs at ~1 min/step)

Stage 1 note: no soup BUILD planned, but adapter saves kept verbatim (cheap insurance,
[[feedback_always_save_soup_adapters]]); resumable checkpoint required.
"""
...
LOG_FILE = f"/root/autodl-tmp/train_log_curriculum_stage1_{'fixtie' if os.environ.get('MOE_TIE', '1') == '1' else 'notie'}{'' if os.environ.get('USE_MEM_EFF', '1') == '1' else '_outproj'}.txt"
...
CSV_PATH   = '/root/autodl-tmp/data/260614_STAGE1_FULL.csv'  # curriculum Stage 1: oversampling=0 gates off aug -> 6450 active -> ~7340 expanded -> ~230 steps
...
_DATA_TAG  = 'curriculum_stage1'  # Stage 1: original-only data (aug gated off via oversampling=0), random init
