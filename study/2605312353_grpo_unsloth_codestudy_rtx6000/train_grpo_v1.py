...
SEED_ADAPTER_PATH = os.environ.get('SEED_ADAPTER', '/root/autodl-tmp/sft_seed_086_moe_outproj')
DATA_CSV          = os.environ.get('DATA_CSV',     '/root/autodl-tmp/data/260607_Cryptarithm_gtTrue.csv')
# TODO before launch: confirm SEED_ADAPTER_PATH points to the 0.86 SFT adapter dir
# Data CSV columns required: id, category, prompt, answer, solver_cot (gold CoT for distillation reward)
# Training is filtered to cryptarithm + equation_numeric categories only (per user — other cats already saturated by SFT 0.86)
...
sys.path.insert(0, os.path.join(os.path.dirname(__file__), '..', '..', '02_train'))
...
# Pre-render each prompt with the chat template + suffix; drop those too long; filter categories
formatted        = []
n_dropped_long   = 0
n_dropped_cat    = 0
n_missing_cot    = 0
for row in rows:
    ...
    gold_cot = row.get('solver_cot', '') or row.get('gold_cot', '')
    if not gold_cot or len(gold_cot.strip()) < 20:
        n_missing_cot += 1; continue
    ...
    formatted.append({
        ...
        'oversampling': int(float(row.get('oversampling', 1) or 1)), # propagated through reward fns
...
print(f'  dropped (no gold_cot): {n_missing_cot}')
print(f'  dropped (prompt long): {n_dropped_long}')
print(f'  kept                 : {len(formatted)}')
print(f'  KEEP_CATEGORIES      : {sorted(KEEP_CATEGORIES)}')
...
if STRESS_TEST:
    ...
    print(f'STRESS_TEST: trimmed to {len(formatted)} rows')
