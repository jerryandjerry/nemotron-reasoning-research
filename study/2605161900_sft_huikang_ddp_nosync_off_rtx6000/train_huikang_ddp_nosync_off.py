#!/usr/bin/env python3
"""
Huikang 0.84 config — DDP 2-GPU, NO_SYNC OFF (always all-reduce every micro-batch).

Background:
- Previous DDP perMBnorm run (2605152045) scored 0.78 vs single-GPU baseline 0.84.
- Step-1 loss matched single-GPU exactly (0.3859) — forward is bit-identical.
- Step-1 grad_norm differed by 14.5% (1.4142 DDP vs 1.2356 single-GPU) — way too
  large for bf16 NCCL reduction noise. Structural bug somewhere in the DDP grad
  pipeline.
- Hypothesis (sharpened by 2nd-agent review): `static_graph=True` captures the
  reducer/bucket map on the FIRST backward call. The first backward in the
  perMBnorm script ran INSIDE `model.no_sync()` (because mb_idx=0 is not the
  last MB). Captured state was wrong → all subsequent reductions inherit the
  wrong bucket layout → 14% grad_norm shift compounds over 244 steps.

Fix (this script): drop `no_sync()` entirely. All-reduce on every backward.
~5% slower (4× the NCCL communication per step), math is unchanged:
    DDP: ((Σ_{i in r0} g_i/4) + (Σ_{i in r1} g_i/4)) / 2  =  Σ_{i=0..7} g_i / 8
which equals single-GPU's Σ_{i=0..7} g_i / 8.

Everything else IDENTICAL to perMBnorm (data, batch, LR, MoE tie, contiguous
rank split, per-MB normalization, Unsloth reentrant ckpt + static_graph=True,
gradient_as_bucket_view=True).

Diagnostic logging: per-group grad-L2 at steps 1-3, logged from rank 0 before
_tie_grads and clip. If step-1 grad_norm matches Sub#4's 1.2356 ± 0.025, fix
works; run full 244 steps.

Save schedule simplified: FIFO=1 full checkpoint every 50 steps + force-save
at final step. No intermediate adapter-only saves (no soup-building this run;
focus is validating the fix).

Launch: torchrun --nproc_per_node=2 train_huikang_ddp_nosync_off.py
"""
...
OUTPUT_DIR = '/root/autodl-tmp/adapter_output_huikang_ddp_nosync_off'
ZIP_PATH   = '/root/autodl-tmp/submission.zip'

PROMPT_SUFFIX = '\nPlease put your final answer inside `\\boxed{}`. For example: `\\boxed{your answer}`'

# Save schedule: FIFO=1 full checkpoint every 50 steps + final.
# No intermediate adapter-only saves (no soup-building this run).
SAVE_STEPS = 50
...
# Diagnostic: log per-group grad-L2 for these steps only (cheap, ~10ms/step)
...
RUN_NAME = f'{_ts}_sft_huikang_ddp2gpu_nosync_off_r32_a32_lr2e4_seq8192_bs32_rtx6000'
...
if IS_MAIN:
    ...
    if _verify_ok:
        ...
        _log('Token verification: some mismatches (warn-only)')
...
# ── Wrap in DDP ─────────────────────────────────────────────────────
# static_graph=True kept (required for Unsloth's reentrant ckpt).
# gradient_as_bucket_view=True kept (the in-place writes in _tie_grads happen
# AFTER all-reduce, so bucket reuse for next-iter accumulation is correct).
# broadcast_buffers=False kept (Nemotron has no trainable buffers needing sync).
model = DDP(
    model,
...
if IS_MAIN:
    print('Wrapped model in DDP (static_graph=True, no_sync OFF)')

# ── Build training order (huikang's index.jsonl) ────────────────────
...
_log(f'\n=== Starting training (NO_SYNC OFF — every MB triggers all-reduce) ===')
...
def _log_grad_groups(prefix, mod):
    """Log per-group grad-L2 norms — diagnostic only, ~10ms."""
...
for step_idx in range(num_steps):
    ...
    # Contiguous rank split: rank 0 = global[0:16], rank 1 = global[16:32]
    ...
    for mb_idx, mb_start in enumerate(range(0, n_rank, MICRO_BATCH_SIZE)):
        ...
        # ── NO no_sync() — let DDP all-reduce every MB ──────────────
    ...
    # Diagnostic: per-group grad-L2 BEFORE _tie_grads + clip
    if (step + 1) in DIAGNOSTIC_GRAD_STEPS:
        _log_grad_groups(f'step={step+1} PRE-TIE', model.module)

    _tie_grads()

    # Diagnostic: per-group grad-L2 AFTER _tie_grads
    if (step + 1) in DIAGNOSTIC_GRAD_STEPS:
    ...
    # ── Checkpoint saving (FIFO=1 full state every SAVE_STEPS + final) ──
    if IS_MAIN and (step % SAVE_STEPS == 0 or step == num_steps):
        ...
        # FIFO=1: delete older checkpoints
        existing = sorted(
...
if IS_MAIN:
    ...
    _log('\n' + '='*60)
    _log('TRAINING SUMMARY — huikang DDP 2-GPU + NO_SYNC OFF')
    _log('='*60)
    _log(f'Method:       LoRA bf16 + CCE + MoE tying + DDP (no_sync OFF)')
