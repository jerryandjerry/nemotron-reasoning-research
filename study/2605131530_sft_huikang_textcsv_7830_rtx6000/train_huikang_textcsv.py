#!/usr/bin/env python3
"""
Huikang 0.85 config with text CSV data (tokenize on the fly).
...
Tokenization matches huikang's corpus.py:
- Prompt: apply_chat_template(enable_thinking=True) — adds <think>\n
- Completion: tokenizer.encode(reasoning + \n</think>\n\boxed{answer}<|im_end|>)
...
CSV_PATH   = '/root/autodl-tmp/data/huikang_7830.csv'
OUTPUT_DIR = '/root/autodl-tmp/adapter_output_huikang_textcsv'
...
RUN_NAME = f'{_ts}_sft_huikang_textcsv7830_r32_a32_lr2e4_seq8192_bs32_rtx6000'
...
# ── Load CSV, tokenize matching huikang's corpus.py ────────────────
import csv, re
...
for row in raw_rows:
    ...
    # apply_chat_template with enable_thinking=True adds <think>\n at end
    ...
    # ── Completion tokenization (matches huikang's corpus.py) ────
    # Extract answer from last \boxed{} in reasoning text
    boxed_matches = re.findall(r'\\boxed\{([^}]*)\}', cot)
    reasoning_answer = boxed_matches[-1] if boxed_matches else answer

    # Build completion: full reasoning + \n</think>\n\boxed{answer}<|im_end|>
    completion_text = f'{cot}\n</think>\n\\boxed{{{reasoning_answer}}}<|im_end|>'
...
# ── Stratified batching (matches huikang's _stratified_batches) ─────
import random as _random

def _stratified_batches(examples, batch_size, rng):
    """Categories distributed evenly across batches, shuffled within each."""
    n = len(examples)
...
if STRESS_TEST:
    # Stress test: sort by length descending, sequential
    batches = None
...
# ── Summary ─────────────────────────────────────────────────────────
_log('\n' + '='*60)
_log('TRAINING SUMMARY — huikang config + text CSV 7830')
_log('='*60)
