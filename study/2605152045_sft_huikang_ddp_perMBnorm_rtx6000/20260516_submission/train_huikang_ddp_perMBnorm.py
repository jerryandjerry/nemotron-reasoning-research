#!/usr/bin/env python3
"""
Huikang 0.84 config — DDP 2-GPU with PER-MB normalization (fix vs 260515 run).

Background:
- Previous DDP run (2605151830) used GLOBAL per-token loss normalization:
    loss = local_ce_sum / global_token_count * WORLD_SIZE
  This gives a "uniform per-token" gradient — mathematically clean.
- The 0.84 single-GPU recipe used PER-MICRO-BATCH normalization:
    loss = mb_ce_sum / mb_token_count, then (loss / n_accum).backward()
  This gives a "macro-average over MBs" gradient. Each micro-batch contributes
  equally regardless of its token count → short-CoT MBs get higher per-token
  gradient weight.
- The two formulas produce IDENTICAL reported loss (per-token mean across batch)
  but DIFFERENT gradients. The 0.84 baseline was tuned with per-MB; the global
  recipe scored 0.73 (-0.11) on Kaggle.
- This run replicates the per-MB normalization under DDP.

DDP math: per-MB loss = mb_ce_sum / mb_weight_sum; divide by n_accum_per_rank
(=4) before backward. DDP's auto /WORLD_SIZE then takes the gradient to exactly
1/(weight * n_accum_per_rank * WORLD_SIZE) = 1/(weight * 8), matching single-GPU.
No *WORLD_SIZE cancellation needed because the normalization is per-MB, not
global.

Launch: torchrun --nproc_per_node=2 train_huikang_ddp_perMBnorm.py
"""
...
# Soup-friendly checkpoint schedule: coarse every 30 steps + dense final 10%.
# All intermediate saves are ADAPTER-ONLY (no optimizer state). Final step gets
# a full checkpoint (with optimizer state) for resume capability.
# Total: 12 adapter-only + 1 full = 13 saves.
ADAPTER_SAVE_STEPS = sorted({
    30, 60, 90, 120, 150, 180, 210,    # coarse every 30
    220, 225, 229, 234, 239,           # dense final 10% (90/92/94/96/98%)
})
# Note: final step (num_steps, typically 244) always gets a FULL checkpoint
# regardless of schedule, via the FinalStepSaveCallback-equivalent inline check.
...
RUN_NAME = f'{_ts}_sft_huikang_ddp2gpu_perMBnorm_r32_a32_lr2e4_seq8192_bs32_rtx6000'
...
for step_idx in range(num_steps):
    ...
    # Contiguous rank split (matches single-GPU MB structure exactly).
    # Rank 0 gets global_batch_indices[0:16], rank 1 gets [16:32].
    # Within each rank, MBs of 4 then match single-GPU's [0:4],[4:8],...,[28:32].
    ...
    # ── Track GLOBAL weight only for reporting (not used in gradient) ──
    ...
    # For reporting: accumulate (ce_sum, weight_sum) across micro-batches.
    # global_mean_loss = global_ce_sum / global_weight, all-reduced.
    ...
    for mb_idx, mb_start in enumerate(range(0, n_rank, MICRO_BATCH_SIZE)):
        ...
        with sync_ctx:
            with torch.amp.autocast('cuda', dtype=torch.bfloat16):
                ...
                # Per-MB mean (matches 0.84 single-GPU recipe).
                # DDP auto-averages across ranks (/WORLD_SIZE), so the effective
                # gradient per MB is 1/(weight_sum * n_accum * WORLD_SIZE)
                # = 1/(weight_sum * 8) — identical to single-GPU's per-MB-mean.
                if weight_sum_t > 0:
    ...
    # ── Checkpoint saving ─────────────────────────────────────────────
    # Two kinds:
    #   - ADAPTER-ONLY at scheduled intermediate steps (for post-hoc soup):
    #     just adapter_model.safetensors + adapter_config.json, ~1.7 GB each.
    #   - FULL checkpoint at the final step (FinalStepSaveCallback equivalent):
    #     adds optimizer state, tokenizer, RNG, training args. ~3 GB.
    # No FIFO removal — we want all 13 checkpoints to survive for soup analysis.
    if IS_MAIN and step in ADAPTER_SAVE_STEPS and step != num_steps:
        ckpt_name = f'adapter-{step}_loss{global_mean_loss:.4f}_lr{lr:.2e}'
        ckpt_dir = os.path.join(OUTPUT_DIR, ckpt_name)
        os.makedirs(ckpt_dir, exist_ok=True)
        model.module.save_pretrained(ckpt_dir)  # PEFT save: adapter only
        _log(f'  Adapter-only saved: {ckpt_name}')

    if IS_MAIN and step == num_steps:
