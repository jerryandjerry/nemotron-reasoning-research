...
RUN_NAME = f'{_ts}_sft_qlora_konbu17_loss_topk32_r32_seq2000_ep2_bs22_rtx6000'

MODEL_PATH = '/root/autodl-tmp/Nemotron-3-Nano-30B-A3B'
DATA_PATH  = '/root/autodl-tmp/data/konbu17_verified_cot_6558rows.csv'
OUTPUT_DIR = '/root/autodl-tmp/adapter_output_loss_topk32'
...
# Top-k worst tokens, k=32. For each sequence, average ONLY the 32 highest-loss
# per-token CE values. Literal "train on the hardest tokens in each trace,
# ignore everything the model already gets right". Crude but directly targets
# the min-logprob critical region without any smoothness.
TOPK_K = 32
...
def weighted_loss_fn(outputs, labels, num_items_in_batch=None):
    ...
    nll = -logp.gather(1, safe.unsqueeze(1)).squeeze(1).view_as(shift)  # [B, T-1]

    mask = (shift != -100)
    # masked positions get 0 NLL so they never appear in top-k
    nll = torch.where(mask, nll, torch.zeros_like(nll))

    k = min(TOPK_K, nll.size(-1))
    topk_vals, _ = nll.topk(k=k, dim=-1)  # [B, k]
    # normalize by number of VALID top-k entries (some may have hit zero-masked padding)
