#!/usr/bin/env python3
"""
Stage 3 rerun: Stage 2 adapter -> crypt10k_FULL data for 2 continuous epochs with WSD.

Continues from Stage 2's adapter via INIT_ADAPTER_FROM env (loads Stage 2 LoRA weights into
fresh LoRA layers; fresh optimizer + fresh scheduler). This tests the planned 2-epoch WSD
schedule while keeping the proven Stage 3 trainer/config otherwise unchanged.

Diff vs 0.87 run (train_crypt10k_FULL_ep2_contS3.py):
  - _DATA_TAG     -> 'crypt10k_FULL_2ep_wsd_contS2'
  - Peak LR       -> 1.5e-4
  - Epochs        -> 2 continuous passes over the same stratified batches
  - Schedule      -> WSD: 5% warmup, stable to 70%, final 30% linear decay
  - clip_grad_norm max_norm 1e9 -> 1.0 (routine required feature)

Recipe otherwise matches the 0.87 run: bf16 + CCE + no-tie + live out_proj +
manual lm_head LoRA, r32 a32, bs=32 (micro=4, ga=8), seq=8192, stratified seed=42,
soup adapters at 20/60/80/85/90/95/100% of num_steps.

Expected at startup:
  CSV rows: 18652; rows with oversampling>0: 18048
  Expanded ~18804 -> ~588 steps/epoch -> ~1176 total steps at bs=32
"""
...
LOG_FILE = f"/root/autodl-tmp/train_log_crypt10k_2ep_wsd_{'fixtie' if os.environ.get('MOE_TIE', '1') == '1' else 'notie'}{'' if os.environ.get('USE_MEM_EFF', '1') == '1' else '_outproj'}.txt"
...
CSV_PATH   = '/root/autodl-tmp/data/260609_Crypt10k_eqHardened_FULL.csv'  # Stage 3: crypt10k FULL -> 18048 train -> 18804 expanded -> ~588 steps
...
_DATA_TAG  = 'crypt10k_FULL_2ep_wsd_contS2'  # same crypt10k FULL data, continues from Stage 2 adapter, 2ep WSD
...
SAVE_STEPS = None  # computed after num_steps per routine
...
RUN_NAME = f'{_ts}_codex_sft_c10k2ep_wsd_contS2_rtx6000'
...
if STRESS_TEST:
    ...
    batches = batches * NUM_EPOCHS
    num_steps = len(batches)
    _log(f'Epochs: {NUM_EPOCHS}, steps_per_epoch={steps_per_epoch}, total_steps={num_steps}')

_log(f'Batching: stratified, {num_steps} steps')

SAVE_STEPS = max(25, round(num_steps / 5 / 25) * 25)
_log(f'Resume checkpoint save_steps: {SAVE_STEPS}')

WARMUP_STEPS = max(1, round(num_steps * WARMUP_RATIO))
DECAY_START_STEP = max(WARMUP_STEPS, round(num_steps * (1.0 - DECAY_RATIO)))
DECAY_STEPS = max(1, num_steps - DECAY_START_STEP)
_log(f'LR schedule: WSD warmup_steps={WARMUP_STEPS}, stable_until_step={DECAY_START_STEP}, '
     f'decay_steps={DECAY_STEPS}, peak_lr={LEARNING_RATE}')
...
def _lr_for_step(step):
    ...
    return LEARNING_RATE * max(0.0, 1.0 - decay_progress)
...
try:
    ...
    swanlab.init(
        ...
        config={
            ...
            'epochs': NUM_EPOCHS, 'schedule': 'wsd_5_65_30',
            'warmup_ratio': WARMUP_RATIO, 'decay_ratio': DECAY_RATIO,
...
_log(f'LR:           peak {LEARNING_RATE} WSD: warmup={WARMUP_STEPS}, stable_until={DECAY_START_STEP}, decay={DECAY_STEPS}')
