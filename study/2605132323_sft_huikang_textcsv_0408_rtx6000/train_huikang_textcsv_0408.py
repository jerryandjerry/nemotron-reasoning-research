#!/usr/bin/env python3
"""
Huikang 0.85 config with text CSV decoded from 04-08 pre-tokenized data.
Exact 7,830 samples: 6,171 base + 1,659 oversampled duplicates.

Config: bf16 + CCE + MoE tying, r32 a32, lr 2e-4 linear decay,
AdamW beta2=0.95, bs=32 micro=4, 9 targets + lm_head, seq=8192.

Tokenization matches huikang's corpus.py EXACTLY:
- Prompt: apply_chat_template(enable_thinking=True)
- Completion: raw Tokenizer from tokenizer.json (NOT AutoTokenizer)
- boxed regex: re.escape(chr(92)+'boxed{') to avoid \\b word boundary trap
...
CSV_PATH   = '/root/autodl-tmp/data/huikang_7830_0408.csv'
TOKENIZER_JSON = '/root/autodl-tmp/data/tokenizer.json'  # huikang's raw tokenizer
OUTPUT_DIR = '/root/autodl-tmp/adapter_output_huikang_textcsv_0408'
ZIP_PATH   = '/root/autodl-tmp/submission.zip'

PROMPT_SUFFIX = '\nPlease put your final answer inside `\\boxed{}`. For example: `\\boxed{your answer}`'
SAVE_STEPS = 50  # ~244 steps / 5 ≈ 49, rounded to 50
...
RUN_NAME = f'{_ts}_sft_huikang_textcsv0408_r32_a32_lr2e4_seq8192_bs32_rtx6000'
...
# Regex for \boxed{} — r'\\b' is word boundary in regex, so use re.escape
_boxed_pat = re.compile(re.escape(chr(92) + 'boxed{') + '([^}]*)' + re.escape('}'))

# ── Verify tokenization matches pre-tokenized data ────────────────
...
for row in raw_rows:
    ...
    boxed_matches = _boxed_pat.findall(cot)
...
# ── Verify full token sequences match pre-tokenized data ───────────
...
for _vid in _verify_ids:
    ...
    if not os.path.exists(_pretok_path) or _vid not in _pid_map:
        continue
    with open(_pretok_path) as _f:
        _rec = json.load(_f)
    _pretok_all = _rec['tokens']
    # Our tokens = [:-1] of full sequence, so compare with pretok[:-1]
    _ours = _pid_map[_vid]['tokens']
    ...
    if _ours == _expected:
        ...
        for _i in range(min(len(_ours), len(_expected))):
            if _ours[_i] != _expected[_i]:
                _log(f'  First diff at pos {_i}: ours={_ours[_i]} pretok={_expected[_i]}')
                break
        _verify_ok = False
if _verify_ok:
    _log('Token verification PASSED — text CSV matches pre-tokenized data')
else:
    _log('WARNING: token verification FAILED — text CSV differs from pre-tokenized!')
...
if STRESS_TEST:
    ...
    # Load huikang's exact training order (epoch 0, unique problem_ids)
    ordered_pids = []
    seen = set()
    ...
    # Build batches in huikang's order
    ordered_indices = [_pid_to_idx[pid] for pid in ordered_pids if pid in _pid_to_idx]
    _log(f'Training order: {len(ordered_indices)} samples from index.jsonl (expect 7830)')
    batches = []
    for i in range(0, len(ordered_indices), BATCH_SIZE):
        batches.append(ordered_indices[i:i + BATCH_SIZE])
    num_steps = len(batches)

_log(f'Batching: {"huikang order" if batches else "sequential (stress test)"}, {num_steps} steps')
...
# ── Summary ─────────────────────────────────────────────────────────
_log('\n' + '='*60)
_log('TRAINING SUMMARY — huikang config + text CSV 0408 decoded')
_log('='*60)
