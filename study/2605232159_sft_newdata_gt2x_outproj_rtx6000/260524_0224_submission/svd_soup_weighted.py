#!/usr/bin/env python3
"""WEIGHTED SVD LoRA soup — dynamic recipes by % of final step. Best rank-r approx of Σ wᵢ·Bᵢ@Aᵢ
(scale each Aᵢ by wᵢ, drop /N; weights sum to 1). OUTPUT_DIR/SUBMIT_DIR env-controlled.
Recipes:  last5-soup = 5 highest, 0.5 on final + 0.125 each on other 4
          wise-soup  = nearest 20/60/100%, weights 0.1/0.2/0.7"""
...
SUBMIT_DIR  = os.environ.get('SUBMIT_DIR', '/root/autodl-tmp/_wsoup_submit')
WORKING_DIR = '/root/autodl-tmp/_svd_work_wsoup'
os.makedirs(SUBMIT_DIR, exist_ok=True); os.makedirs(WORKING_DIR, exist_ok=True)

step_to_dir = {}
for d in sorted(os.listdir(OUTPUT_DIR)):
    full = os.path.join(OUTPUT_DIR, d)
    if os.path.isdir(full) and d.startswith('adapter-'):
        try: step_to_dir[int(d.split('-')[1].split('_')[0])] = full
        except (IndexError, ValueError): pass
print(f'OUTPUT_DIR={OUTPUT_DIR}')
print(f'Found {len(step_to_dir)} adapters: {sorted(step_to_dir.keys())}')

first_dir = step_to_dir[sorted(step_to_dir.keys())[0]]
with open(os.path.join(first_dir, 'adapter_config.json')) as f: cfg = json.load(f)
...
# dynamic weighted recipes (by % of the final saved step)
_steps = sorted(step_to_dir.keys()); _final = _steps[-1]
def _nearest(frac): return min(_steps, key=lambda s: abs(s - frac * _final))
_last5 = _steps[-5:]
_last5_w = {s: (0.5 if s == _final else 0.125) for s in _last5}
_wise_w = {_nearest(0.20): 0.1, _nearest(0.60): 0.2, _final: 0.7}
SOUPS = [('last5-soup', _last5_w), ('wise-soup', _wise_w)]
print(f'Recipes: last5-soup={_last5_w}  wise-soup={_wise_w}')


def svd_merge_qr_weighted(Bs, As, weights, rank):
    """best rank-r approx of  Σ wᵢ Bᵢ@Aᵢ  (scale Aᵢ by wᵢ, no /N)."""
    Bs = [B.to(DEVICE, dtype=torch.float32) for B in Bs]
    As = [w * A.to(DEVICE, dtype=torch.float32) for w, A in zip(weights, As)]
    B_stack = torch.cat(Bs, dim=1); A_stack = torch.cat(As, dim=0)
...
def rename_lm_head(tensors):
    ...
    for k, v in tensors.items():
        if k.startswith(src): out[dst + k[len(src):]] = v; n += 1
        else: out[k] = v
    print(f'  renamed {n} lm_head keys'); return out


def patch_config(src, dst):
    with open(src) as f: c = json.load(f)
    c['base_model_name_or_path'] = 'metric/nemotron-3-nano-30b-a3b-bf16'; c['inference_mode'] = True
    with open(dst, 'w') as f: json.dump(c, f, indent=2)


for name, wmap in SOUPS:
    steps = sorted(wmap.keys()); ws = [wmap[s] for s in steps]
    ...
    if missing: print(f'  SKIP: missing {missing}'); continue
    t0 = time.time()
    ...
    for a_key in lora_a_keys:
        ...
        if b_key not in base_keys: print(f'  WARN: no B for {a_key}'); continue
        As = [td[a_key] for td in all_t]; Bs = [td[b_key] for td in all_t]
        orig_dtype = As[0].dtype
        t1 = time.time()
        new_B, new_A = svd_merge_qr_weighted(Bs, As, ws, rank=RANK)
        t_svd += time.time() - t1
        new_tensors[a_key] = new_A.to(orig_dtype); new_tensors[b_key] = new_B.to(orig_dtype); n_done += 1
    ...
    with zipfile.ZipFile(zip_path, 'w', zipfile.ZIP_DEFLATED) as zf:
        zf.write(sf_path, arcname='adapter_model.safetensors'); zf.write(cfg_path, arcname='adapter_config.json')
    with zipfile.ZipFile(zip_path) as zf: bad = zf.testzip()
...
for f in sorted(glob.glob(os.path.join(SUBMIT_DIR, 'submission_*soup.zip'))):
