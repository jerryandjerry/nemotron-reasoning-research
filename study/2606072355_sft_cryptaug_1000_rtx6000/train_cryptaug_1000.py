#!/usr/bin/env python3
"""
Fresh SFT on 260607_Cryptarithm_1000_FULL.csv (huikang-golden base + 260607_NE_aug (765 eq aug) +
260607_crypt_aug (693 crypt aug) + 260606_new_solver eq (621) + 260601_new_solver crypt (307);
all GT-True training rows; 0 rows over the 7680 token cap — augmentation kept CoTs under cap).

Diff vs the bitman_failcorr trainer (per feedback_copy_sft_verbatim — verbatim except CSV swap):
  - CSV_PATH      -> /root/autodl-tmp/data/260607_Cryptarithm_1000_FULL.csv (md5 928065b844f19297ce144541e40620ad)
  - _DATA_TAG     -> 'cryptaug_1000'
  - Stress-test out_proj.lora_B grad assert (per TRAINING_ROUTINE §1 rule 6) — fails fast if the
    fused Mamba kernel folded out_proj (silent 0.84 cap).

Recipe (byte-identical to #16): bf16 + CCE + no-tie + live out_proj + manual lm_head LoRA,
r32 a32, lr 2e-4 linear-to-0, bs=32 (micro=4, ga=8), seq=8192, stratified seed=42,
soup adapters at 20/60/80/85/90/95/100% of num_steps.

Expected at startup (verify against data audit before launching real training):
  CSV rows (unique): 7931
  Expanded ~8687 -> ~272 steps at bs=32 (~4.5 hrs at ~1 min/step)
"""
...
CSV_PATH   = '/root/autodl-tmp/data/260607_Cryptarithm_1000_FULL.csv'  # huikang base + 260607_NE_aug + 260607_crypt_aug + 260606+260601 solver -> 7931 train -> 8687 expanded -> ~272 steps
...
_DATA_TAG  = 'cryptaug_1000'  # 260607 augmented cryptarithm + numeric-equation overlay on huikang base
