#!/usr/bin/env python3
"""
Huikang 0.84 config + lkevincc golden cryptarithm data.
6,906 unique rows → 7,849 expanded (via oversampling column).

Same training setup as 0.84 run: bf16 + CCE + MoE tying, r32 a32,
lr 2e-4 linear decay, AdamW beta2=0.95, bs=32 micro=4,
9 targets + lm_head, seq=8192.

Data: 6,171 original 0408 samples + 735 golden cryptarithm solutions.
"""
...
CSV_PATH   = '/root/autodl-tmp/data/260514_huikang_golden.csv'
TOKENIZER_JSON = '/root/autodl-tmp/data/tokenizer.json'  # huikang's raw tokenizer
OUTPUT_DIR = '/root/autodl-tmp/adapter_output_huikang_golden'
...
RUN_NAME = f'{_ts}_sft_huikang_golden_r32_a32_lr2e4_seq8192_bs32_rtx6000'
...
try:
    ...
    swanlab.init(
        ...
        config={
            ...
            'data': 'huikang_0408 + lkevincc_golden_cryptarithm',
        },
    )
    _swanlab_ok = True
...
# ── Summary ─────────────────────────────────────────────────────────
_log('\n' + '='*60)
_log('TRAINING SUMMARY — huikang 0408 + golden cryptarithm')
_log('='*60)
