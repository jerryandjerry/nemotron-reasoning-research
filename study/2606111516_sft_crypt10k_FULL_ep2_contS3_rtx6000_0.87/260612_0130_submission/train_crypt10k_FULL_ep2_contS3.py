#!/usr/bin/env python3
"""
Stage 4 = Stage 3 + another epoch (continual SFT). Same CSV as Stage 3
(260609_Crypt10k_eqHardened_FULL.csv); continues from Stage 3's adapter (Sub #33 = 0.84,
the over-fit signal). Tests whether Stage 3 was under-fit or over-fit by running
one more epoch at fresh 2e-4 -> 0 LR.

Continues from Stage 3's adapter via INIT_ADAPTER_FROM env (loads Stage 3 LoRA weights into
fresh LoRA layers; fresh optimizer + fresh scheduler — clean 1-epoch cycle at lr 2e-4 -> 0).

Diff vs Stage 3 (train_crypt10k_FULL_contS2.py) — verbatim except:
  - _DATA_TAG     -> 'crypt10k_FULL_ep2_contS3'   (CSV unchanged; only OUTPUT_DIR + ZIP_PATH naming change)
  - Launch env: INIT_ADAPTER_FROM=/root/autodl-tmp/stage3_adapter_init (Stage 3's adapter).

Recipe (byte-identical to #16 / #30 / Stage 1 / Stage 2 / Stage 3): bf16 + CCE + no-tie +
live out_proj + manual lm_head LoRA, r32 a32, lr 2e-4 linear-to-0, bs=32 (micro=4, ga=8),
seq=8192, stratified seed=42, soup adapters at 20/60/80/85/90/95/100% of num_steps.
...
_DATA_TAG  = 'crypt10k_FULL_ep2_contS3'  # Stage 4: same crypt10k FULL data, continues from Stage 3 adapter (epoch 2)
