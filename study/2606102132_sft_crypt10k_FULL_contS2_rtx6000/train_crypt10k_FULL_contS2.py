#!/usr/bin/env python3
"""
Stage 3 SFT on 260609_Crypt10k_eqHardened_FULL.csv (Stage 2 data + +7,307 more crypt10k
cryptarithm rows — escalates the "strengthen cryptarithm" lever further: 10,000 crypt10k
rows total (vs Stage 2's 2,693), cryptarithm_deduce now 9,639 raw (vs 3,041)).
...
Diff vs Stage 2 (train_eqHard_FULL_contS1.py) — verbatim except:
  - CSV_PATH      -> /root/autodl-tmp/data/260609_Crypt10k_eqHardened_FULL.csv
                     (md5 0654f4639b0b51c0c01f5fe0edebf4dd)
  - _DATA_TAG     -> 'crypt10k_FULL_contS2'

Recipe (byte-identical to #16 / #30 / Stage 1 / Stage 2): bf16 + CCE + no-tie + live out_proj +
...
Expected at startup:
  ...
  INIT_ADAPTER_FROM should point at the directory containing Stage 2's
  adapter_model.safetensors + adapter_config.json (extracted from Stage 2's submission.zip).
"""
...
_DATA_TAG  = 'crypt10k_FULL_contS2'  # Stage 3: crypt10k FULL, continues from Stage 2 adapter (eqHard_FULL_contS1)
