#!/usr/bin/env python3
"""
LoRA Training on bf16 model (NOT quantized) on RTX PRO 6000 (95GB VRAM).
Same config as QLoRA train but with full bf16 precision base model.
"""
...
GRAD_ACCUM   = 8
LR           = 1e-4
# Generate run name with Chicago CDT timestamp
...
# Stress test: keep only 2000-3000 token samples (worst case VRAM)
hf_dataset = hf_dataset.filter(lambda x: 2000 <= x['token_len'] <= 3000, desc='Stress test filter 2000-3000')
...
# Enable gradient checkpointing (no prepare_model_for_kbit_training for bf16)
...
print('='*60)
print(f'Method:      LoRA bf16 (full precision, NOT quantized)')
print(f'GPU:         {torch.cuda.get_device_name(0)}')
