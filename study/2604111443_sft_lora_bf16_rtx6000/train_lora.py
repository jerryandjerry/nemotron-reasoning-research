...
# trl 1.1.0's SFTTrainer breaks transformers' DefaultFlowCallback final-step
# auto-save (it sets `control.should_save=True` when global_step>=max_steps).
# Without this callback the run leaves no `checkpoint-{final_step}` subfolder
# even though the final adapter is what we actually want to ship.
