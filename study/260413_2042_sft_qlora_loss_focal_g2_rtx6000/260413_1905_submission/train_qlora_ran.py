...
RUN_NAME = f'{_ts}_sft_qlora_konbu17_loss_focal_g2_r32_seq2000_ep2_bs22_rtx6000'

MODEL_PATH = '/root/autodl-tmp/Nemotron-3-Nano-30B-A3B'
DATA_PATH  = '/root/autodl-tmp/data/konbu17_verified_cot_6558rows.csv'
OUTPUT_DIR = '/root/autodl-tmp/adapter_output_loss_focal_g2'
...
# Focal loss, gamma=2. Per-token CE multiplied by (1-p)^gamma, where p is the
# model's probability on the correct next token. Confident tokens (p~1) get
# near-zero weight; uncertain tokens (p~0) get full weight. Dynamic approximation
# of "maximize min logprob" (THK's Open Progress Prize objective framing).
FOCAL_GAMMA = 2.0
...
def weighted_loss_fn(outputs, labels, num_items_in_batch=None):
    ...
    # p = exp(-nll) is the model's prob on the correct token
    p = nll.neg().exp().clamp(min=1e-8, max=1.0)
    focal = (1.0 - p).clamp_min(1e-8).pow(FOCAL_GAMMA)

    loss = (focal * nll * mask).sum() / mask.sum().clamp_min(1)
    return loss
