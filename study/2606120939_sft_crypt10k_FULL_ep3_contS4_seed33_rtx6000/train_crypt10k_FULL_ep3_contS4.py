#!/usr/bin/env python3
"""
Stage 5 = Stage 4 + another epoch (continual SFT), NEW SHUFFLE SEED 33. Same CSV as
Stage 3/4 (260609_Crypt10k_eqHardened_FULL.csv); continues from Stage 4's adapter
(Sub #34 = 0.86). Tests whether more epochs keep helping (S3->S4 was +0.02) with a
fresh batch order (epochs 1+2 both used seed=42 order; seed=33 avoids a third
identical pass) at fresh 2e-4 -> 0 LR.

Continues from Stage 4's adapter via INIT_ADAPTER_FROM env (loads Stage 4 LoRA weights into
fresh LoRA layers; fresh optimizer + fresh scheduler — clean 1-epoch cycle at lr 2e-4 -> 0).

Diff vs Stage 4 (train_crypt10k_FULL_ep2_contS3.py) — verbatim except:
  - _DATA_TAG     -> 'crypt10k_FULL_ep3_contS4'   (CSV unchanged; only OUTPUT_DIR + ZIP_PATH naming change)
  - SHUFFLE_SEED  -> 33  (user-requested: new batch order for epoch 3)
  - Launch env: INIT_ADAPTER_FROM=/root/autodl-tmp/stage4_adapter_init (Stage 4's adapter).

Recipe (byte-identical to #16 / #30 / Stage 1 / Stage 2 / Stage 3 / Stage 4): bf16 + CCE + no-tie +
live out_proj + manual lm_head LoRA, r32 a32, lr 2e-4 linear-to-0, bs=32 (micro=4, ga=8),
seq=8192, stratified seed=33, soup adapters at 20/60/80/85/90/95/100% of num_steps.

Expected at startup:
  CSV rows: 18652; rows with oversampling>0: 18048
  Expanded ~18804 -> ~588 steps at bs=32 (~9.8 hrs at ~1 min/step)
"""
...
_DATA_TAG  = 'crypt10k_FULL_ep3_contS4'  # Stage 5: same crypt10k FULL data, continues from Stage 4 adapter (epoch 3, seed 33)
