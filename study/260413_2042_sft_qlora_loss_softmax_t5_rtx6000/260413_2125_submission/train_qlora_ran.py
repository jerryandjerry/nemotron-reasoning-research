...
RUN_NAME = f'{_ts}_sft_qlora_konbu17_loss_softmax_t5_r32_seq2000_ep2_bs22_rtx6000'

MODEL_PATH = '/root/autodl-tmp/Nemotron-3-Nano-30B-A3B'
DATA_PATH  = '/root/autodl-tmp/data/konbu17_verified_cot_6558rows.csv'
OUTPUT_DIR = '/root/autodl-tmp/adapter_output_loss_softmax_t5'
...
# Soft-max per sequence via log-sum-exp with tau=5. Smooth approximation of
# per-sequence max CE — the loss becomes biased toward whichever tokens in
# each trace currently have the highest NLL (lowest logprob). As tau grows,
# this converges to literal "minimize max-NLL per trace".
SOFTMAX_TAU = 5.0
...
def weighted_loss_fn(outputs, labels, num_items_in_batch=None):
    ...
    nll_flat = -logp.gather(1, safe.unsqueeze(1)).squeeze(1)
    nll = nll_flat.view_as(shift)            # [B, T-1]

    mask = (shift != -100)
    # masked positions: push to very negative so they can never dominate the soft-max
    nll_masked = torch.where(mask, nll, torch.full_like(nll, -1e9))
    # soft-max per sequence: (1/tau) * logsumexp(tau * nll)
    per_seq = torch.logsumexp(SOFTMAX_TAU * nll_masked, dim=-1) / SOFTMAX_TAU
    # guard: sequences with zero valid tokens produce -inf, drop them
    valid_seq = mask.any(dim=-1)
    if valid_seq.any():
        loss = per_seq[valid_seq].mean()
    else:
        loss = nll_flat.sum() * 0.0
    return loss
...
ckpt_dirs = sorted([d for d in os.listdir(OUTPUT_DIR) if d.startswith('checkpoint-')], key=lambda x: int(x.split('-')[1].split('_')[0]))
if ckpt_dirs:
