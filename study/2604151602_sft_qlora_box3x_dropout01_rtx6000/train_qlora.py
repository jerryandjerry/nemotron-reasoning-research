#!/usr/bin/env python3
"""
QLoRA training on AutoDL RTX PRO 6000 — konbu17 0.72 recipe + Unsloth.

Training config copied verbatim from study/2604121408_sft_lora_konbu17_rtx6000:
  - konbu17 data recipe (konbu17_verified_cot_6558rows.csv, type-balanced sampling)
  - CoT cleaned to strip existing \\boxed{...}
  - Prompt format with </think> close + \\boxed{...} example suffix
  - rank 32, alpha 32, seq 2000, 2 epochs, bs 4 ga 2, lr 1e-4,
    warmup_ratio 0.05, cosine, adamw_torch
  - target_modules: Mamba in_proj/out_proj + non-expert FFN up_proj/down_proj
    (MoE expert down_proj caught via the same suffix regex)
...
RUN_NAME = f'{_ts}_sft_qlora_konbu17_box3x_dropout01_r32_seq2000_ep2_bs22_rtx6000'

MODEL_PATH = '/root/autodl-tmp/Nemotron-3-Nano-30B-A3B'
DATA_PATH  = '/root/autodl-tmp/data/konbu17_verified_cot_6558rows.csv'
OUTPUT_DIR = '/root/autodl-tmp/adapter_output_box3x_dropout01'
ZIP_PATH   = '/root/autodl-tmp/submission_loss_boxed5x.zip'
...
model = FastLanguageModel.get_peft_model(
    ...
    lora_dropout   = 0.1,  # triggers unsloth slow fallback, but adds regularization
    bias           = 'none',
    use_gradient_checkpointing = 'unsloth',
    random_state   = 3407,
    use_rslora     = False,
...
# ── Custom loss function for this variant ──────────────────────────
import torch.nn.functional as F

# Kh0a weighted boxed-answer loss: per-token CE, but every token from the
# last occurrence of "\boxed{" to end-of-sequence gets weight 5.0.
# The boxed region is the eval-critical region — wrong token = wrong submission.
BOXED_LOSS_WEIGHT = 3.0
_BOXED_TOKEN_IDS = None  # lazy init once tokenizer exists

def _find_last_boxed_start(row_ids, needle):
    n = len(needle)
    L = row_ids.size(0)
    if n == 0 or L < n:
        return -1
    ...
    for i in range(L - n + 1):
        if bool((row_ids[i:i+n] == needle).all()):
            last = i
    return last

def weighted_loss_fn(outputs, labels, num_items_in_batch=None):
    global _BOXED_TOKEN_IDS
    if _BOXED_TOKEN_IDS is None:
        ids = tokenizer.encode("\\boxed{", add_special_tokens=False)
        _BOXED_TOKEN_IDS = torch.tensor(ids, device=outputs.logits.device)
    ...
    nll = -logp.gather(1, safe.unsqueeze(1)).squeeze(1).view_as(shift)

    weights = torch.ones_like(nll)
    needle = _BOXED_TOKEN_IDS.to(shift.device)
    for b in range(shift.size(0)):
        row = shift[b]
        # replace masked positions with -1 so they never match the needle
        row_clean = torch.where(mask.view_as(shift)[b], row, torch.full_like(row, -1))
        pos = _find_last_boxed_start(row_clean, needle)
        if pos >= 0:
            weights[b, pos:] = BOXED_LOSS_WEIGHT

    m = mask.view_as(shift).float()
    loss = (nll * weights * m).sum() / (weights * m).sum().clamp_min(1.0)
    return loss
...
training_args = SFTConfig(
    ...
    save_strategy               = 'steps',
    save_steps                  = 50,   # ~265 total steps / 5 ≈ 53, rounded to 50
    save_total_limit            = 2,
    report_to                   = 'swanlab',
    run_name                    = RUN_NAME,
...
adapter_config['lora_dropout'] = 0.1
