#!/usr/bin/env python3
"""
Stage 2 SFT on 260609_Crypt3k_eqHardened_FULL.csv (Stage 1 data + aug-row oversampling of
cryptarithm + numeric_equation rows — the "strengthen targeted categories" stage).

Continues from Stage 1's adapter via INIT_ADAPTER_FROM env (loads Stage 1 LoRA weights into
fresh LoRA layers; fresh optimizer + fresh scheduler — clean 1-epoch cycle at lr 2e-4 -> 0).

Diff vs Stage 1 (train_eqHard_AUG0x.py) — verbatim except:
  - CSV_PATH      -> /root/autodl-tmp/data/260609_Crypt3k_eqHardened_FULL.csv
                     (md5 46b787fd33eea30d3766d72ea45a5e89)
  - _DATA_TAG     -> 'eqHard_FULL_contS1'
  - INIT_ADAPTER_FROM env block (after LoRA wrap + lm_head LoRA + fp32 cast):
    if set, load adapter_model.safetensors from that path and overwrite the freshly
    created LoRA weights with Stage 1's trained weights. Same `.default.` rename as
    the routine §2.4A resume block. Optimizer + scheduler stay fresh (no resume).

Recipe (byte-identical to #16 / #30 / Stage 1): bf16 + CCE + no-tie + live out_proj +
manual lm_head LoRA, r32 a32, lr 2e-4 linear-to-0, bs=32 (micro=4, ga=8), seq=8192,
stratified seed=42, soup adapters at 20/60/80/85/90/95/100% of num_steps.

Expected at startup:
  CSV rows: 11345; rows with oversampling>0: 10741
  Expanded ~11497 -> ~359 steps at bs=32 (~5.9 hrs at ~1 min/step)
  INIT_ADAPTER_FROM should point at the directory containing Stage 1's
  adapter_model.safetensors + adapter_config.json (extracted from Stage 1's submission.zip).
"""
...
CSV_PATH   = '/root/autodl-tmp/data/260609_Crypt3k_eqHardened_FULL.csv'  # Stage 2: eqHardened, FULL oversampling -> 10741 train -> 11497 expanded -> ~359 steps
...
_DATA_TAG  = 'eqHard_FULL_contS1'  # Stage 2: eqHardened FULL oversampling, continues from Stage 1 adapter
